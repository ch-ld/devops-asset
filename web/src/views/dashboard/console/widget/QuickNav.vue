<template>
  <div class="quick-nav-ant">
    <div class="nav-group-bar">
      <div class="nav-group-tabs">
        <button
          ...
          :key="group"
          :class="['ant-btn', 'ant-btn-round', currentGroup === group ? 'ant-btn-primary' : 'ant-btn-default']"
            ...
            <div class="nav-card-header">
              ...
              <div class="nav-more ant-dropdown" @click.stop="toggleMenu(nav.id)">
                ...
                <div class="dropdown-menu" v-show="menuOpenId === nav.id">
                  <div class="dropdown-item" @click.stop="onEditNav(nav)"><i class="anticon anticon-edit"></i> 编辑</div>
                  <div class="dropdown-item danger" @click.stop="onDeleteNav(nav.id)"><i class="anticon anticon-delete"></i> 删除</div>
                </div>
              </div>
            </div>
          </div>
    ...
    <!-- 导航编辑弹窗 -->
    <NavEditModal ref="navEditModalRef" :group-options="groupTabs.filter(g => g !== '便捷导航')" @refresh="loadNavData" />
    <NavManageModal
...
const groupTabs = computed(() => {
  const groups = navList.value.map(nav => nav.group || '未分组')
...
const loadNavData = async () => {
  try {
    ...
    console.error('导航数据加载异常:', error)
    ElMessage.error('加载导航数据失败')
  }
}

// 智能获取 logo，带 favicon 缓存
function getNavLogo(nav) {
...
function onManageEdit(nav) { onEditNav(nav) }
function onManageDelete(id) { onDeleteNav(id) }
function onManageAdd() {
  navEditModalRef.value?.showModal('add')
}
function onManageMove({from, to}) {
  // 参数验证
  if (from === to) return
...
function onManageBatchDelete(ids) {
...
function handleClickOutside(e) {
  if (!e.target.closest('.nav-more')) menuOpenId.value = null
}
...
.quick-nav-ant {
  ...
  .anticon {
    ...
    &.anticon-setting:before { content: '\2699'; }
    &.anticon-edit:before { content: '\270E'; }
    &.anticon-delete:before { content: '\1F5D1'; }
    &.anticon-ellipsis:before { content: '\2026'; }
  }
}
</style> 
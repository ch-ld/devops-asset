...
<script setup lang="ts">
  ...
  const openTerminal = () => {
    ...
    message.info('打开终端功能正在开发中')
  }

  // 打开SFTP
  const openSftp = () => {
    // 实现打开SFTP的逻辑
    message.info('打开文件管理功能正在开发中')
  }
  ...
  const confirmRestart = () => {
    Modal.confirm({
      title: '确认重启主机',
      content: '确定要重启该主机吗？这可能会导致正在运行的服务中断。',
      okText: '确认重启',
      okType: 'danger',
      cancelText: '取消',
      onOk: async () => {

<template>
  <div class="cert-create">
    <el-page-header @back="goBack" content="申请证书">
      <template #extra>
        <el-space>
    ...
    <el-card class="mt-4">
      <el-steps :active="currentStep" finish-status="success" class="mb-6">
        <el-step title="选择方式" description="选择证书申请方式" />
        <el-step title="配置信息" description="配置证书相关信息" />
        <el-step title="高级设置" description="部署和监控设置" />
        <el-step title="确认申请" description="确认信息并提交申请" />
      </el-steps>
        ...
        <el-form :model="formData" label-width="120px">
          <el-form-item label="申请方式">
            <el-radio-group v-model="formData.applyType" @change="handleApplyTypeChange">
              <el-radio label="auto" class="radio-card">
                <div class="radio-content">
                  ...
                  <div class="radio-desc">
                    使用Let's Encrypt自动申请和管理证书，支持自动续期
                  </div>
                </div>
              </el-radio>
              <el-radio label="csr" class="radio-card">
                <div class="radio-content">
                  <div class="radio-title">
      ...
      <div v-show="currentStep === 3" class="step-content">
        ...
        <el-descriptions title="证书信息" :column="2" border>
          <el-descriptions-item label="申请方式">
            {{ formData.applyType === 'auto' ? '自动申请' : '自定义CSR' }}
          </el-descriptions-item>
...
import { Lightning, Upload, UploadFilled, Plus } from '@element-plus/icons-vue'
...
const fetchDomainOptions = async () => {
  try {
    ...
    domainOptions.value = response.data
...
const fetchHostOptions = async () => {
  try {
    ...
    hostOptions.value = response.data
  } catch (error) {
...
const handleDomainChange = (domainId: number) => {
  ...
  if (domain) {
    ...
    formData.providerId = domain.providerId
  }
}
...
const handleApply = async () => {
  try {
    ...
    if (formData.applyType === 'csr') {
      ...
      await certificateApi.issueWithCSR({
        csrContent: formData.csrContent,
        caType: formData.caType,
      ...
      await certificateApi.create({
        ...
        deployHosts: formData.deployHosts,
        deployPath: formData.deployPath,
        restartCommand: formData.restartCommand,
    ...
    ElMessage.error('证书申请失败: ' + (error.message || '未知错误'))
  } finally {

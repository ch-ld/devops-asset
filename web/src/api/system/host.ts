...
import type { Provider, Host, HostGroup, BatchOperationResult, SSHResult, SFTPResult, HostAlert } from '@/types/api/host'
...
export function syncResources(providerId: number) {
  return client.post(`${base}/providers/${providerId}/sync`)
}
...
export function getHostList(params?: {
  ...
  return client.get<{ total: number; data: Host[] }>(`${base}/hosts`, { params })
}
...
export function createManualHost(data: Partial<Host>) {
  return client.post(`${base}/hosts/manual`, data)
}

export function moveHost(id: number, groupId?: number) {
  return client.post(`${base}/hosts/${id}/move`, null, {
...
export function batchSFTP(data: {
  ids: number[]
  remote_path: string
  file: File
}) {

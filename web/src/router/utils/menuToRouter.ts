 ...
 * @returns 处理后的路由配置
 */
export function menuDataToRouter(menu: any, parentPath = ''): any {
  // 拼接绝对路径
  let fullPath = menu.path;
  if (!fullPath.startsWith('/')) {
    fullPath = parentPath ? `${parentPath.replace(/\/$/, '')}/${fullPath.replace(/^\//, '')}` : `/${fullPath.replace(/^\//, '')}`;
  }
  const newMenu = { ...menu, path: fullPath };
  if (Array.isArray(menu.children) && menu.children.length > 0) {
    newMenu.children = menu.children.map(child => menuDataToRouter(child, fullPath));
  } else {
    newMenu.children = [];
  }
  return newMenu;

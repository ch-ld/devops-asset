...
const handleSubmit = async () => {
  ...
  try {
    ...
    const submitData: any = {
      name: form.name.trim(),
      registrar_type: form.registrar_type,
      status: form.status,
    ...
    // 处理expires_at，确保格式正确
    if (form.expires_at) {
      // 如果是字符串，直接使用；如果是Date对象，转换为ISO字符串
      submitData.expires_at = typeof form.expires_at === 'string'
        ? form.expires_at
    ...
    // 处理group_id，确保类型正确
    if (form.group_id && form.group_id !== '') {
      const groupId = Number(form.group_id)
      if (!isNaN(groupId) && groupId > 0) {
        submitData.group_id = groupId
      } else {
    ...
    ElMessage.error(isEdit.value ? `域名更新失败: ${errorMsg}` : `域名创建失败: ${errorMsg}`)
  } finally {
    loading.value = false
  }
}

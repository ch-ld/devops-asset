...
const downloadTemplate = async (format: 'xlsx' | 'csv') => {
  try {
    ...
    const blob = await domainApi.downloadTemplate(apiFormat as 'csv' | 'excel')

    // 创建下载链接
    const url = window.URL.createObjectURL(blob)
    const link = document.createElement('a')
    link.href = url
    link.download = `域名导入模板.${format}`
...
const handleImport = async () => {
  try {
    ...
    importResult.value = {
      success: (result as any).data?.success || (result as any).success,
      failed: (result as any).data?.failed || (result as any).failed,
      total: (result as any).data?.total || (result as any).total,
      failed_items: (result as any).data?.failed_items || (result as any).failed_items || []
    }
    currentStep.value = 3

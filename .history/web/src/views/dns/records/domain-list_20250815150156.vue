...
const fetchDomainRecordCounts = async () => {
  if (!domainList.value || domainList.value.length === 0) return
  const ids = (domainList.value as any[]).map(d => d.id)
  try {
    console.log('正在获取域名记录数，域名IDs:', ids)
    const resp = await recordApi.countByDomain(ids)
    console.log('记录数API响应:', resp)
    const map: Record<string, number> = resp || {}
    domainList.value = (domainList.value as any[]).map(row => ({
      ...row,
      record_count: map[String(row.id)] ?? 0
    }))
    console.log('更新后的域名列表:', domainList.value)
  } catch (e) {
    // 出错时不打断主流程，保持为 0
    console.error('获取域名记录数失败', e)
    ElMessage.error('获取记录数失败: ' + (e as any)?.message)
  }
}
...
// 单个域名导出解析记录
const exportDomainRecords = async (domain: any) => {
  try {
    ...
    a.href = url
    a.download = `${domain.name}-records.csv`

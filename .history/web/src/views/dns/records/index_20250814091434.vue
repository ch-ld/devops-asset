    ...
    <div class="table-container">
      <el-card shadow="never">
        ...
        >
          ...
          <el-table-column prop="domain" label="域名" min-width="150">
            <template #default="{ row }">
              <div class="domain-cell">
                <span class="domain-name">{{ row.domain?.name || '未知域名' }}</span>
                <div v-if="row.domain?.name" class="domain-id">ID: {{ row.domain_id }}</div>
              </div>
            </template>
          </el-table-column>
          ...
          <el-table-column prop="provider" label="DNS提供商" width="120">
            <template #default="{ row }">
              <div v-if="row.provider" class="provider-cell">
                <el-tag type="info" size="small">{{ row.provider.name }}</el-tag>
              </div>
              <span v-else>-</span>
            </template>
          </el-table-column>
...
// 选择
const selectedRows = ref<DnsRecord[]>([])

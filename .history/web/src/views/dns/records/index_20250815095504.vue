  ...
  <div class="dns-records-page">
    ...
    <RecordModal
      ...
      :record="currentRecord"
                :domain-id="selectedDomain || undefined"
...
// 新增方法 - 视图模式切换
const handleViewModeChange = (mode: string | number | boolean | undefined) => {
  ...
  if (mode === 'detail') {
    if (selectedDomain.value) {
      ...
      // 如果切换到详情模式但没有选中域名，重置记录列表
      records.value = []
      selectedRecords.value = []
    }
  } else if (mode === 'list') {
    // 切换到列表模式时，重新获取域名列表确保数据最新
    fetchDomains()
    // 重置选中状态
    selectedRecords.value = []
...
const initializeFromRoute = () => {
  ...
  const domainIdParam = route.query.domainId as string
  const viewModeParam = route.query.viewMode as string
  
  // 优先处理viewMode参数
  if (viewModeParam) {
    viewMode.value = viewModeParam
  }
  
  if (domainIdParam) {
    // 如果有domainId参数，设置选中域名
    selectedDomain.value = parseInt(domainIdParam)
    
    // 根据viewMode决定是否切换到详情模式
    if (viewModeParam === 'list') {
      viewMode.value = 'list'
      // 确保域名列表是最新的
      fetchDomains()
    } else {
    ...
    // 如果有domain参数，先查找对应的域名ID
    fetchDomains().then(() => {
      const domain = domains.value.find(d => d.name === domainParam)
      if (domain) {
        selectedDomain.value = domain.id
        if (viewModeParam !== 'list') {
          viewMode.value = 'detail'
...
watch(() => route.query, (newQuery) => {
  if (newQuery.domain || newQuery.domainId) {
    initializeFromRoute()
  }
}, { immediate: false })

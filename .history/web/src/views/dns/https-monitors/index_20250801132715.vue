      ...
      >
        ...
        <el-table-column label="响应时间" width="100">
          <template #default="{ row }">
           ...
           <template #default="{ row }">
             <el-tag v-if="row.httpStatus" :type="getHttpStatusType(row.httpStatus) as any">
               {{ row.httpStatus }}
             </el-tag>
             <span v-else>-</span>
           </template>
           ...
           <template #default="{ row }">
             <div v-if="row.sslInfo">
               <el-tag :type="getCertStatusType(row.sslInfo.status) as any">

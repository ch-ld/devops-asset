              ...
              <el-dropdown trigger="click" @command="(command: string) => handleCommand(command, row)">
                ...
                <template #dropdown>
                  <el-dropdown-menu class="action-dropdown">
                    <el-dropdown-item command="sync" :disabled="row.status !== 'active'">
                      <el-icon class="dropdown-icon"><Refresh /></el-icon>
                      <span>同步域名</span>
                    </el-dropdown-item>
                    <el-dropdown-item command="toggle">
                      ...
                      <span>{{ row.status === 'active' ? '禁用提供商' : '启用提供商' }}</span>
                    </el-dropdown-item>
...
.action-dropdown .danger-item:hover {
  background: #fff2f0;
  color: #ff4d4f;
}

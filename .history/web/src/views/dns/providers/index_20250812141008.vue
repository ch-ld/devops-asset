        ...
        <el-table-column label="创建时间" width="180">
          <template #default="{ row }">
            <div class="time-info">
              ...
              <!-- 调试信息 -->
              <div class="debug-info" v-if="true" style="font-size: 10px; color: #999;">
                原始数据: {{ row.created_at }} (类型: {{ typeof row.created_at }})
              </div>
            </div>
          </template>
        </el-table-column>
...
const fetchProviderList = async () => {
  ...
  try {
    ...
    if (data.items && data.items.length > 0) {
      console.log('DNS提供商数据调试信息:', {
        firstItem: data.items[0],
        created_at: data.items[0].created_at,
        created_at_type: typeof data.items[0].created_at,
        formatted_time: formatTime(data.items[0].created_at)
      })
    }

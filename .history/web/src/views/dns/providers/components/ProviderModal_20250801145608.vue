  ...
  try {
    ...
    if (props.mode === 'edit' && props.provider?.id) {
      ...
      const testData = {
        ...
        remark: formData.remark || 'Test connection'
      }
...
const buildCredentials = () => {
  ...
  switch (formData.type) {
    case 'aliyun':
      credentials.accessKeyId = formData.accessKeyId
      credentials.accessKeySecret = formData.accessKeySecret
      credentials.region = formData.region || 'cn-hangzhou'
      break
    case 'cloudflare':
      credentials.apiToken = formData.apiToken
      credentials.zoneId = formData.zoneId
      break
    case 'route53':
      credentials.accessKeyId = formData.accessKeyId
      credentials.secretAccessKey = formData.secretAccessKey

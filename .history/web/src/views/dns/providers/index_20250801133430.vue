    ...
    providerList.value = [
      {
        ...
        type: 'aliyun',
        status: 'enabled',
        description: '阿里云域名解析服务',
        domainCount: 15,
        createdAt: new Date().toISOString(),
        lastTestResult: { success: true, message: '连接正常' }
      },
      {
        id: 2,
        name: 'Cloudflare',
        ...
        lastTestResult: { success: false, message: 'API密钥错误' }
      }
    ]
...
const handleToggleStatus = async (record: any) => {
  try {
    // TODO: 调用API切换状态
    ElMessage.success(`${record.status === 'enabled' ? '禁用' : '启用'}成功`)
    fetchProviderList()
  } catch (error) {

    ...
    <div class="page-header">
      <div class="page-header-content">
        ...
        <div class="page-actions">
          <el-button @click="handleSyncAllDomains" :loading="syncLoading" class="page-action-btn sync-btn">
            <el-icon v-if="!syncLoading"><Refresh /></el-icon>
            <span>{{ syncLoading ? '同步中...' : '一键同步域名' }}</span>
          </el-button>
        ...
        <div class="table-header">
          ...
          <div class="table-actions">
            ...
            <div class="action-group secondary-group">
              <el-button @click="handleSyncAllDomains" class="modern-btn sync-btn" :loading="syncLoading">
                <el-icon v-if="!syncLoading"><Refresh /></el-icon>
                <span>{{ syncLoading ? '同步中...' : '一键同步域名' }}</span>
              </el-button>
              <el-button @click="handleTestAll" class="modern-btn test-all-btn">
                <el-icon><Connection /></el-icon>
                <span>批量测试</span>
              </el-button>
            </div>
            
            <div class="action-group danger-group" v-if="selectedRowKeys.length > 0">
              <el-button 
                type="danger" 
...
const formatTime = (time: string | number) => {
  ...
  try {
    ...
    if (date.getFullYear() < 2020) {
      console.warn('时间可能异常（早于2020年）:', time, '解析结果:', date.toISOString())
      return '时间可能异常'
    }
...
.page-action-btn.sync-btn {
  background: linear-gradient(135deg, #52c41a, #73d13d);
  color: white;
}

.page-action-btn.sync-btn:hover {
...
.provider-table .el-table__body tr {
  transition: all 0.3s ease;
  border-radius: 8px;
}

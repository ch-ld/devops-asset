    ...
    <div class="main-content">
      <el-form ref="formRef" :model="formData" :rules="formRules" label-width="120px" class="cert-form">
        ...
        <div class="form-section">
          ...
          <div class="section-content">
            <el-form-item prop="caType">
              <div class="ca-provider-group">
                <div
                  v-for="provider in caProviders"
                  :key="provider.type"
                  class="ca-provider-option"
                  :class="{ 'is-checked': formData.caType === provider.type }"
                  @click="formData.caType = provider.type"
                  @keydown.enter.prevent="formData.caType = provider.type"
                  role="button"
                  tabindex="0"
                >
                  <!-- 隐藏的radio用于保持表单一致性与可验证性 -->
                  <el-radio v-model="formData.caType" :label="provider.type" style="display:none" />
...
const loadDomains = async () => {
  try {
    ...
    availableDomains.value = response.data?.items || []
  } catch (error) {

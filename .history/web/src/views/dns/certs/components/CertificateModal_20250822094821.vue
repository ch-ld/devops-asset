        ...
        <div v-show="currentStep === 0" class="step-content">
          ...
          >
            <el-form-item label="证书类型" prop="cert_type">
              <el-radio-group v-model="formData.cert_type" class="cert-type-group">
                <el-radio-button value="auto">自动申请</el-radio-button>
                <el-radio-button value="upload">上传证书</el-radio-button>
              </el-radio-group>
            </el-form-item>

            <template v-if="formData.cert_type === 'auto'">
              <el-form-item label="主域名" prop="domains">
                <el-input
                  v-model="primaryDomain"
                  placeholder="请输入主域名，如：example.com"
                  @input="updateDomains"
                />
                <div class="form-tip">证书的主要域名，必须填写</div>
              </el-form-item>

              <el-form-item label="备用域名">
                <el-tag
                  v-for="domain in altDomains"
                  :key="domain"
                  closable
                  @close="removeAltDomain(domain)"
                  class="domain-tag"
                >
                  {{ domain }}
                </el-tag>
                <el-input
                  ...
                  size="small"
                  @keyup.enter="addAltDomain"
                  @blur="addAltDomain"
                  class="alt-input"
                />
                <el-button v-else size="small" @click="showAltInputDialog" class="add-domain-btn">
                  + 添加域名
                </el-button>
                <div class="form-tip">可选的备用域名，如：www.example.com, api.example.com</div>
              </el-form-item>
            :value="item"
          />
        </el-select>

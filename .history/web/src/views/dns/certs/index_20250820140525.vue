...
const handleDownload = async (row: Certificate) => {
  try {
    ...
    const formatOptions = [
      {
        ...
        value: 'pem',
        icon: 'Document',
        desc: '标准PEM格式，适用于大多数数据库服务器',
        detail: '包含证书文件(.crt)和私钥文件(.key)'
      },
      {
        label: 'Nginx配置',
        value: 'nginx',
      ...
      },
      {
        ...
        icon: 'Setting',
        desc: 'Apache服务器配置文件',
        detail: '包含apache配置示例和证书文件'
      },
      {
        label: 'IIS配置',
        value: 'iis',
        icon: 'Monitor',
        desc: 'Windows IIS服务器配置',
        detail: '包含pfx格式证书文件'
      },
      {
        label: 'Tomcat配置',
    ...
    const { value: format } = await ElMessageBox({
      title: '选择下载格式',
      message: h('div', { class: 'download-format-selector' }, [
        h('div', { class: 'cert-info' }, [
          h('p', { style: 'margin: 0 0 16px 0; color: #606266; font-size: 14px;' },

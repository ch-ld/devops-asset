<template>
  <div class="dns-cert-container">
    ...
    <el-drawer
      ...
      :title="`证书详情 - ${currentCertificate?.common_name || currentCertificate?.domain_name || '未知域名'}`"
      size="50%"
    >
      <CertificateDetail
...
const handleDownload = async (row: Certificate) => {
  try {
    ...
    // 使用更简单的选择方式
    const formatOptions = [
      { label: 'PEM格式 (通用)', value: 'pem' },
      { label: 'Nginx配置', value: 'nginx' },
      { label: 'Apache配置', value: 'apache' },
      { label: 'IIS配置', value: 'iis' },
      { label: 'Tomcat配置', value: 'tomcat' },
      { label: '私钥文件', value: 'key' },
      { label: '证书文件', value: 'crt' },
      { label: '证书链文件', value: 'chain' }
    ]

    // 创建选择列表的HTML
    const optionsHtml = formatOptions.map(opt =>
      `<div style="padding: 8px; cursor: pointer; border-radius: 4px;"
           onmouseover="this.style.backgroundColor='#f5f7fa'"
           onmouseout="this.style.backgroundColor=''"
           onclick="window.selectedFormat='${opt.value}'; document.querySelector('.el-message-box__btns .el-button--primary').click()">
         ${opt.label}
       </div>`
    ).join('')
    ...
    await ElMessageBox({
      title: '选择下载格式',
      message: `<div style="max-height: 300px; overflow-y: auto;">${optionsHtml}</div>`,
      dangerouslyUseHTMLString: true,
      confirmButtonText: '取消',
      showCancelButton: false,
      beforeClose: (action, instance, done) => {
        if ((window as any).selectedFormat) {
          done()
        } else {
          done()
        }
    ...
    const format = (window as any).selectedFormat || 'pem'
    ...
    ElMessage.success(`证书${format.toUpperCase()}格式下载成功`)
  } catch (error) {
    if (error !== 'cancel') {

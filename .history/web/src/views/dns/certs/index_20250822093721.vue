...
import { getCertificates, deleteCertificate, updateCertificate } from '@/api/dns/certificate'
...
const handleAutoRenewChange = async (cert, value) => {
  try {
    await updateCertificate(cert.id, { auto_renew: value })
...
const handleRenewCert = (cert) => {
  ElMessage.info('手动续期功能开发中...')
}
...
const getExpiringDays = (expiresAt) => {
  if (!expiresAt) return ''
...
const getExpiresClass = (expiresAt) => {
  if (!expiresAt) return ''

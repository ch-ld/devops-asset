<!-- 这个文件已被临时禁用，请使用 create-new-simple.vue -->
<template>
  <div>
    <h1>此页面已被禁用</h1>
    <p>请使用新的证书申请页面</p>
          ...
          <div class="step-section" v-if="formData.applyType === 'auto'">
            ...
            <el-form-item label="选择域名" prop="domainId" required>
              <el-select
                ...
                :placeholder="formData.domains.length > 0 ? `已选择域名：${formData.domains[0]}` : '请选择要申请证书的域名'"
                filterable
              ...
              <!-- 已选择域名显示 -->
              <div v-if="formData.domains.length > 0" class="selected-domain-display">
                <el-tag
                  type="success"
                  size="large"
                  closable
                  @close="clearDomainSelection"
                  style="margin-bottom: 12px;"
                >
                  <el-icon><Connection /></el-icon>
                  {{ formData.domains[0] }}
                </el-tag>
                <span class="domain-tip">点击 × 可重新选择域名</span>
              </div>

              <!-- 自定义域名输入 -->
              <div v-if="formData.domains.length === 0" class="custom-domain-section">
                <el-input
                ...
                <div class="domain-examples">
                  ...
                  <el-tag
                    ...
                    size="small"
                    class="example-tag"
                    @click="useExampleDomain(example)"
                  >
                    {{ example }}
                  </el-tag>
                </div>
              ...
              <div class="form-tip">
                ...
                可选择已有域名或输入自定义域名，支持通配符域名（*.domain.com）和多级域名
              </div>
            </el-form-item>

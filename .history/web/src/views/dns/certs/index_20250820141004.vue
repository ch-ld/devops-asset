  ...
  try {
    ...
    const { value: format } = await ElMessageBox.prompt(
      ...
      '下载证书',
      {
        ...
        inputType: 'select',
        inputOptions: {
          'pem': 'PEM格式 (通用) - 标准格式，适用于大多数服务器',
          'nginx': 'Nginx配置 - 包含Nginx配置示例',
          'apache': 'Apache配置 - 包含Apache配置示例',
          'iis': 'IIS配置 - Windows IIS服务器配置',
          'tomcat': 'Tomcat配置 - 包含JKS格式证书',
          'key': '私钥文件 - 仅下载私钥(.key)',
          'crt': '证书文件 - 仅下载证书(.crt)',
          'chain': '证书链 - 完整证书链文件'
        },

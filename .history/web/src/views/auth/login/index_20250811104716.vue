...
<script setup lang="ts">
      ...
      if (valid) {
        ...
        try {
          ...
          if (res.code === 200 && res.data) {
            ...
            // 等待路由注册完成后再跳转
            console.log('登录成功，等待路由注册完成...')
            await waitForRouteRegistration()
            
            // 跳转首页
            console.log('路由注册完成，跳转首页')
            router.push('/')
          } else {
  ...
  // 等待路由注册完成
  const waitForRouteRegistration = async () => {
    ...
    return new Promise<void>((resolve) => {
      const checkRoutes = () => {
        ...
        const hasRoutes = router.hasRoute('Dashboard') || 
                         router.hasRoute('Console') ||
                         router.getRoutes().some(route => route.path.startsWith('/dashboard'))
        
        if (hasRoutes) {
          console.log('检测到路由已注册，可以跳转')
          resolve()
        } else if (waitTime >= maxWait) {
          console.log('等待路由注册超时，强制跳转')
          resolve()
        } else {
          waitTime += checkInterval
          setTimeout(checkRoutes, checkInterval)
        }
      }

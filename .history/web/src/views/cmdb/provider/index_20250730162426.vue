...
async function showGroupSelectDialog(row: any) {
  ...
  // 重置同步配置
  syncConfig.regions = []
  syncConfig.groupId = undefined

  // 获取主机组列表和区域列表
  await Promise.all([
    fetchHostGroups(),
    fetchProviderRegions(row.id)
  ])
...
// 获取云账号的可用区域
async function fetchProviderRegions(providerId: number) {
  if (!providerId) return

  loadingRegions.value = true
  try {
    const response = await getProviderRegionsById(providerId)

    // 处理响应数据，转换为选择器需要的格式
    if (response.code === 200 && Array.isArray(response.data)) {
      availableRegions.value = response.data.map((region: string) => ({
        value: region,
        label: getRegionLabel(region)
      }))
    } else {
      availableRegions.value = []
      ElMessage.warning('获取区域列表失败，请检查云账号配置')
    }
  } catch (error) {
...
function handleRegionFocus() {
  if (currentSyncProvider.value && availableRegions.value.length === 0) {
    fetchProviderRegions(currentSyncProvider.value.id)
  }
}
...
async function handleSync(row: any, regions: string[], groupId?: number) {
  // 防止重复同步
  ...
  try {
    ...
    // 显示开始同步的消息
    const regionInfo = regions.length > 1 ? `${regions.length}个区域` : regions[0]
    const groupInfo = groupId ? `到指定主机组` : ''
    ElMessage.info(`开始同步云账号 "${row.name}" 在 ${regionInfo} 的资源${groupInfo}...`)

    // 调用同步接口
    const response = await syncResources(row.id, regions, groupId)
    const result = response.data || response

    // 根据同步结果显示不同的消息
    if (result.success) {
      // 构建详细的同步结果消息
      let detailMessage = `${result.message}<br/>云账号: ${result.provider_name || row.name}<br/>`

      if (result.details && result.details.length > 0) {
        detailMessage += '区域详情:<br/>'
        result.details.forEach((detail: any) => {
          const status = detail.success ? '✅' : '❌'
          detailMessage += `${status} ${detail.region}: ${detail.count}台主机<br/>`
        })
      }

      detailMessage += `总耗时: ${Math.round(result.duration / 1000000)}ms`
...
async function confirmSync() {
  ...
  // 验证必填项
  if (!syncConfig.regions || syncConfig.regions.length === 0) {
    ElMessage.warning('请至少选择一个区域进行同步')
    return
  }

  groupSelectDialog.value = false
  await handleSync(currentSyncProvider.value, syncConfig.regions, syncConfig.groupId)
}

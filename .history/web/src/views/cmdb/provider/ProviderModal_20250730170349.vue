...
const needsRegion = computed(() => {
  return ['alicloud', 'tencentcloud', 'aws'].includes(formState.value.type)
})
...
function getRegionLabel(region: string): string {
  // AWS区域保持原始英文名称，不进行中文映射
  if (formState.type === 'aws') {
    return region
  }
...
/* 确保选择框使用默认样式，不受input wrapper样式影响 */

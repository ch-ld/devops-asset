      ...
      <div class="main-content">
        ...
        <div class="table-container">
          ...
          >
            ...
            <!-- 主机名称 -->
            <el-table-column prop="name" label="主机名称" width="120" show-overflow-tooltip>
              <template #default="{ row }">
                <div class="host-name-compact">
            ...
            <el-table-column label="IP地址" width="130">
              <template #default="{ row }">
                <div class="ip-info-optimized">
                  <div class="ip-item-optimized" v-if="getDisplayIP(row.public_ip)">
                    <span class="ip-value-optimized">{{ getDisplayIP(row.public_ip) }}</span>
                    <el-button
                      type="text"
                      size="mini"
                      @click="copyToClipboard(getDisplayIP(row.public_ip))"
                      title="复制公网IP"
                      class="copy-btn-optimized"
                    >
                      <i class="el-icon-copy-document"></i>
                    </el-button>
                  </div>
                  <div class="ip-item-optimized" v-if="getDisplayIP(row.private_ip)">
                    <span class="ip-value-optimized private">{{ getDisplayIP(row.private_ip) }}</span>
                    <el-button
                      type="text"
                      size="mini"
                  ...
                  <div class="ip-item-optimized" v-if="!getDisplayIP(row.public_ip) && !getDisplayIP(row.private_ip)">
            ...
            <el-table-column label="配置" width="90" align="center">
              <template #default="{ row }">
                <div class="config-specs-optimized">
                  <div class="spec-item-optimized">{{ formatConfiguration(row.configuration).cpu }}</div>
                  <div class="spec-item-optimized">{{ formatConfiguration(row.configuration).memory }}</div>
                  <div class="spec-item-optimized">{{ formatConfiguration(row.configuration).disk }}</div>
                </div>
              </template>
            </el-table-column>

            <!-- 系统 -->
            <el-table-column prop="os" label="系统" width="90" show-overflow-tooltip>
              <template #default="{ row }">
            ...
            <!-- 云厂商 -->
            <el-table-column label="厂商" width="70" align="center">
              <template #default="{ row }">
            ...
            <el-table-column label="操作" width="200" align="center" fixed="right">
              <template #default="{ row }">
                <div class="action-buttons-optimized">
                  ...
                  <el-button type="info" size="small" @click="handleSftp(row)" title="SFTP文件管理">
...
@media (max-width: 1400px) {
  ...
  /* 在中等屏幕下隐藏次要列 */
  .host-table :deep(.hide-on-medium) {

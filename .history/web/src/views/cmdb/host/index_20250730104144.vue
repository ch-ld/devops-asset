            ...
            <div class="form-section">
              ...
              <div class="section-content">
                ...
                <el-row :gutter="24" v-if="hostFormData.host_type === 'cloud'">
                  ...
                  <el-col :span="8">
                    <el-form-item label="云账号ID" prop="provider_id">
                      <el-input-number
                        v-model="hostFormData.provider_id"
                        placeholder="云账号ID"
                        size="large"
                        style="width: 100%"
                        :min="1"
                ...
                <el-row :gutter="24">
                  ...
                  <el-col :span="8">
                    <el-form-item label="过期时间" prop="expired_at">
                      <el-date-picker
                        v-model="hostFormData.expired_at"
                        type="datetime"
                        placeholder="选择过期时间"
                  ...
                  <el-col :span="12">
                    <el-form-item label="SSH密码" prop="ssh_config.password">
                      <el-input
                        ...
                        type="password"
                        :placeholder="isEditMode ? '如需修改密码请重新输入' : '请输入SSH密码'"
                        size="large"
                        show-password
                        @input="isPasswordModified = true"
                      />
                      <div v-if="isEditMode && !isPasswordModified" class="password-hint">
                    ...
                    <el-col :span="24">
                      <el-form-item label="私钥内容" prop="ssh_private_key">
                        <el-input
                          ...
                          type="textarea"
                          :rows="6"
                          :placeholder="isEditMode ? '如需修改私钥请重新输入' : '请粘贴SSH私钥内容，或点击下方按钮上传密钥文件'"
                          size="large"
                          @input="isPrivateKeyModified = true"
                        />
                        <div v-if="isEditMode && !isPrivateKeyModified" class="password-hint">
                          <el-text type="info" size="small">
                            <i class="el-icon-info"></i>
                            当前已设置私钥，如不修改请保持为空
                          </el-text>
    ...
    >
      <el-form :model="groupFormData" label-width="100px" class="group-form">
        ...
        <el-form-item label="父分组">
          <el-select v-model="groupFormData.parent_id" placeholder="选择父分组" clearable teleported>
            <el-option
    ...
    >
      <el-form :model="groupFormData" label-width="100px" class="group-form">
        ...
        <el-form-item label="父分组">
          <el-select v-model="groupFormData.parent_id" placeholder="选择父分组" clearable teleported>
            <el-option
              v-for="group in flatGroups.filter(g => g.id !== currentEditGroup?.id)"
              :key="group.id"
              :label="group.name"
              :value="group.id"
            />
...
import { parseIPs } from '@/utils/ipSelection'
...
const hostFormData = reactive({
  ...
  host_group: '', // 主机组ID
  provider_type: 'aws',
  provider_id: null, // 云厂商账号ID
  resource_type: 'manual', // 资源类型
  public_ip: [''],
  private_ip: [''],
  os: '',
  region: '',
...
// filteredGroups 计算属性已移除，由 ModernHostGroupSidebar 组件处理
...
const refreshGroups = async () => {
  ...
  try {
    ...
    // 如果没有分组数据，可能需要触发默认分组创建
    if (hostStore.hostGroupTree.length === 0) {
      // 可以在这里添加创建默认分组的逻辑
    }
    ElMessage.success('分组列表刷新成功')
  } catch (error) {
    ElMessage.error('获取主机分组失败')
  } finally {
    groupLoading.value = false
  }
}

const refreshHosts = async (showMessage = false, resetPage = false) => {
  try {
    const params = {
...
const handleGroupSelect = (group) => {
  ...
  // 根据分组筛选主机，重置页码
  refreshHosts(false, true)
}

const handleAdd = () => {
  // 重置修改标记
  isPasswordModified.value = false
  isPrivateKeyModified.value = false
...
const handleHostTypeChange = (value) => {
  if (value === 'self-hosted') {
    // 自建服务器，清空云厂商相关字段
    hostFormData.provider_type = 'manual'
    hostFormData.resource_type = 'manual'
    hostFormData.provider_id = null
    hostFormData.configuration.instance_type = ''
  } else {
    // 云服务器，设置默认值
    hostFormData.provider_type = 'aws'
    hostFormData.resource_type = 'ec2'
...
const saveHost = async () => {
  try {
    ...
    const saveData = {
      ...
      // 主机类型相关
      resource_type: hostFormData.resource_type || 'manual',
      provider_type: hostFormData.provider_type || 'manual',
      provider_id: hostFormData.provider_id || null,
      ...
      // SSH配置 - 优化：只有在新增模式或用户真的修改了认证信息时才提交
      ...
      // 时间信息
      expired_at: hostFormData.expired_at || null,
...
const handleTerminal = (row) => {
  // 检查主机是否有可用的IP地址
  const publicIPs = parseIPs(row.public_ip)
  const privateIPs = parseIPs(row.private_ip)

  if (publicIPs.length === 0 && privateIPs.length === 0) {
    ElMessage.error('该主机没有配置任何可用的IP地址，请先配置IP地址')
    return
  }
...
const handleEdit = (row) => {
  ...
  Object.assign(hostFormData, {
    ...
    resource_type: row.resource_type || 'manual',
...
const handleDelete = async (row) => {
  try {
    ...
    refreshHosts(false, false) // 不显示消息，不重置页码
  } catch (error) {
    if (error !== 'cancel') {
...
const handleEditGroup = (group: HostGroup) => {
  // 确保分组数据已就绪（编辑场景通常已加载，但保险起见）
  hostStore.ensureHostGroupsReady()
...
const handleBatchOperationSuccess = async () => {
  ...
  setTimeout(async () => {
    await refreshHosts(false, false) // 不显示消息，不重置页码
  }, 500)
}
...
const handleSizeChange = async (size: number) => {
  pageSize.value = size
  currentPage.value = 1
  // 重新加载数据以应用新的分页大小，重置到第一页
  await refreshHosts(false, true)
}

const handleCurrentChange = async (page: number) => {
  currentPage.value = page
  // 重新加载数据以应用新的页码，不重置页码
  await refreshHosts(false, false)
}
...
onMounted(async () => {
  ...
  await refreshGroups()
  await refreshHosts(false, true) // 不显示消息，重置页码
...
const originalPasswordPlaceholder = '••••••••••••••••••••'
const originalPrivateKeyPlaceholder = '密钥已设置，如需修改请重新输入'
</script>
...
.ip-copy-btn-card:hover {
  background: linear-gradient(135deg, #85ce61 0%, #67c23a 100%);
  transform: translateY(-1px);
  box-shadow: 0 3px 6px rgba(103, 194, 58, 0.4);
}

.ip-copy-btn-card:active {
  transform: translateY(0);
}

.ip-copy-btn-card .el-icon {
  font-size: 11px;
}

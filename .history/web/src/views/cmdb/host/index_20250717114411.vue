    ...
    <a-card :bordered="false" class="host-list-card">
      ...
      <a-alert v-if="loadError" type="error" show-icon style="margin-bottom: 16px">
        ...
        <template #description>
          <span>请检查网络连接或服务器状态，然后</span>
          <a-button type="link" size="small" @click="fetchHosts">重试</a-button>
        </template>
      </a-alert>
      ...
      <a-table
        ...
        :row-key="(record) => record?.id || Math.random().toString()"
        @change="handleTableChange"
      >
        <template #bodyCell="{ column, record }">
          <!-- 主机名称列 -->
          <template v-if="column.key === 'name'">
            <div class="host-name-column" v-if="record">
              <a-tooltip :title="record.instance_id">
                <a @click="viewHostDetail(record.id)">{{ record.name || '未命名主机' }}</a>
              </a-tooltip>
          ...
          <template v-else-if="column.key === 'ip'">
            <div class="ip-address-column" v-if="record">
          ...
          <template v-else-if="column.key === 'configuration'">
            <div class="configuration-column" v-if="record && record.configuration">
          ...
          <template v-else-if="column.key === 'os'">
            <div class="os-column" v-if="record">
          ...
          <template v-else-if="column.key === 'status'">
            <a-tag v-if="record" :color="getStatusColor(record.status)">
          ...
          <template v-else-if="column.key === 'expired_at'">
            <div class="expired-at-column" v-if="record">
          ...
          <template v-else-if="column.key === 'created_at'">
            <div class="time-info" v-if="record">
          ...
          <template v-else-if="column.key === 'group'">
            <div class="group-column" v-if="record">
          ...
          <template v-else-if="column.key === 'action'">
            <div class="action-column" v-if="record">
              <a-space>
        ...
        <template #emptyText>
          <a-empty
            :description="loadError ? '加载失败' : '暂无数据'"
            :image="loadError ? 'error' : 'default'"
          >
            <template #extra>

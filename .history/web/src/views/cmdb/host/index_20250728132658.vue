        ...
        <div class="table-container">
          ...
          >
            ...
            <el-table-column label="IP地址" min-width="160">
              <template #default="{ row }">
                <div class="ip-info-compact">
                  <div class="ip-item-compact" v-if="getDisplayIP(row.public_ip)">
                    <span class="ip-label-compact">公:</span>
                    <span class="ip-value-compact">{{ getDisplayIP(row.public_ip) }}</span>
                    <el-button
                      type="text"
                      size="mini"
                  ...
                  <div class="ip-item-compact" v-if="getDisplayIP(row.private_ip)">
                    <span class="ip-label-compact">私:</span>
                    <span class="ip-value-compact">{{ getDisplayIP(row.private_ip) }}</span>
                    <el-button
                      type="text"
                      size="mini"
                  ...
                  <div class="ip-item-compact" v-if="!getDisplayIP(row.public_ip) && !getDisplayIP(row.private_ip)">
            ...
            <el-table-column label="配置" width="120" align="center">
              <template #default="{ row }">
                <div class="config-specs-compact">
                  <div class="spec-line">{{ formatConfiguration(row.configuration).cpu }}</div>
                  <div class="spec-line">{{ formatConfiguration(row.configuration).memory }}</div>
                  <div class="spec-line">{{ formatConfiguration(row.configuration).disk }}</div>
                </div>
              </template>
            </el-table-column>

            <!-- 系统 - 在中等屏幕下隐藏 -->
            <el-table-column prop="os" label="系统" width="100" show-overflow-tooltip class-name="hide-on-medium">
              <template #default="{ row }">
            ...
            <!-- 地域 - 在中等屏幕下隐藏 -->
            <el-table-column prop="region" label="地域" width="100" class-name="hide-on-medium">
              <template #default="{ row }">
            ...
            <!-- 云厂商 -->
            <el-table-column label="厂商" width="80" align="center">
              <template #default="{ row }">

...
const refreshGroups = async () => {
  ...
  try {
    console.log('🔄 开始获取主机分组数据...')
    await hostStore.fetchHostGroupTree()
    console.log('✅ 主机分组数据获取成功:', hostStore.hostGroupTree)
    
    // 如果没有分组数据，可能需要触发默认分组创建
    if (hostStore.hostGroupTree.length === 0) {
      console.log('⚠️ 没有分组数据，可能需要创建默认分组')
      // 可以在这里添加创建默认分组的逻辑
    }
    
    console.log('📊 当前分组数据:', hostStore.hostGroupTree)
...
// 调试：实时观察扁平化后的分组结果
watchEffect(() => {
  console.log('[index.vue] flatGroups', flatGroups.value)
})
</script>

...
// 格式化IP地址显示
const formatIPDisplay = (ipData: string | null) => {
  if (!ipData) return '-'
  ...
  try {
    ...
    if (typeof ipData === 'string' && ipData.startsWith('[')) {
      ...
      return Array.isArray(ips) ? ips.join(', ') : ipData
    }
    return ipData
  } catch {
    return ipData
...
const downloadTemplate = () => {
  const templateData = [
    ...
    // 示例数据行1 - 云主机示例（需要先配置云账号）
    [
      '-', 'i-bp1234567890abcdef', 'web-server-01', '生产环境', 'ecs', 'cn-beijing',
      'root', 'password123', '["1.2.3.4"]', '["10.0.0.1"]', '{"cpu_cores":2,"memory_size":4,"instance_type":"ecs.c6.large"}', 'CentOS 7',
      'running', '2024-12-31 23:59:59', '云主机示例（需要配置云账号）'
    ],
    // 示例数据行2 - 云主机示例（需要先配置云账号）
    [
      '-', 'ins-1234567890', 'db-server-01', '测试环境', 'cvm', 'ap-shanghai',
  ...
  const instructionData = [
    ...
    ['4. 云厂商ID：自建主机留空，云主机填写"-"或有效ID'],
    ['5. 资源类型：manual=自建，ecs=阿里云，cvm=腾讯云，ec2=AWS']
  ]
...
const handleImport = async () => {
  try {
    ...
    // 直接传递文件给后端，让后端处理解析
    const response = await batchImportHosts(selectedFile.value)

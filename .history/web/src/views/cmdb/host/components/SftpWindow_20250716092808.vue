...
<script setup>
  ...
  import {
    ...
    FolderOpened as FolderOpenIcon
  ...
  const listFiles = async () => {
    ...
    try {
      const response = await fetch(
        `/api/v1/cmdb/sftp/list?host_id=${currentHost.value}&path=${encodeURIComponent(currentPath.value)}`,
        {
          headers: {
  ...
  const confirmDelete = (file) => {
    ...
    ElMessageBox.confirm(`确定要删除${fileType} "${file.name}" 吗？`, `确认删除${fileType}`, {
  ...
  const deleteFile = async (file) => {
    ...
    try {
      const response = await fetch(
        `/api/v1/cmdb/sftp/delete?host_id=${currentHost.value}&path=${encodeURIComponent(file.path)}`,
        {
          method: 'DELETE',
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`

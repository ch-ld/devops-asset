...
const primaryIP = computed(() => {
  ...
  if (selectedConnectionParams.value) {
    const type = selectedConnectionParams.value.ipType === 'public' ? '公网' : '私网'
    return `${selectedConnectionParams.value.ip} (${type})`
  }
...
const connectWebSocket = () => {
  ...
  // 如果没有可用IP
  if (analysis.publicIPs.length === 0 && analysis.privateIPs.length === 0) {
    ElMessage.error('该主机没有配置任何可用的IP地址')
    return
  }

  // 如果需要用户选择IP
  if (analysis.needsSelection && !selectedConnectionParams.value) {
    ipSelectionDialog.value?.open()
    return
  }

  // 自动选择或使用已选择的连接参数
  let connectionParams = selectedConnectionParams.value
  if (!connectionParams) {
    connectionParams = autoSelectConnectionIP(props.host)
    if (!connectionParams) {
      ElMessage.error('无法确定连接IP地址')
      return
    }
  }
  ...
  const wsUrl = `${protocol}//${backendHost}/api/v1/ws/ssh?host_id=${props.host.id}`
  ...
  websocket.onopen = () => {
    ...
    if (terminal) {
      const terminalSettings = {
        ...
        preferred_ip_type: connectionParams.preferredIPType,
        preferences: {
          font_size: 14,
          theme: 'dark',
          fullscreen: false,
      ...
      websocket?.send(JSON.stringify(terminalSettings))
    }

...
const primaryIP = computed(() => {
  ...
  // 尝试解析PublicIP
  try {
    const publicIPs = JSON.parse(props.host.publicIP || '[]')
    if (Array.isArray(publicIPs) && publicIPs.length > 0) {
      return publicIPs[0]
    }
  } catch (e) {
    // 如果解析失败，尝试直接使用
    if (props.host.publicIP) return props.host.publicIP
  }
  
  // 尝试解析PrivateIP
  try {
    const privateIPs = JSON.parse(props.host.privateIP || '[]')
    if (Array.isArray(privateIPs) && privateIPs.length > 0) {
      return privateIPs[0]
    }
  } catch (e) {
    // 如果解析失败，尝试直接使用
    if (props.host.privateIP) return props.host.privateIP
  }
...
const startPingMeasurement = () => {
  pingMeasurementInterval = setInterval(() => {
    if (websocket && websocket.readyState === WebSocket.OPEN) {
      lastPingTime.value = Date.now()
      // 发送WebSocket ping帧
      try {

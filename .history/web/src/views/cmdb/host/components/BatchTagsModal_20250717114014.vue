...
<script setup lang="ts">
  ...
  const loadAllTags = async () => {
    ...
    try {
      ...
      // 使用any类型避免类型错误
      const hosts = (response as any)?.data?.data || []
        ...
        hosts.forEach((host: any) => {
          if (host && host.tags) {
            if (Array.isArray(host.tags)) {
              ...
              try {
                const parsedTags = JSON.parse(host.tags)
      ...
      ElMessage.error('加载标签失败，将使用默认标签')
      // 使用常用标签作为备选
      tagOptions.value = commonTags.value.map((tag) => ({
        value: tag,
        label: tag
      }))
    } finally {
  ...
  const handleSubmit = async () => {
    ...
    try {
      // 调用批量标签API
      await hostStore.batchUpdateTags(props.hostIds, formState.selectedTags, formState.actionType)

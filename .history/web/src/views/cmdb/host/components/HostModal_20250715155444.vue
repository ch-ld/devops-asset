      ...
      <a-tabs v-model:activeKey="activeTab">
        <a-tab-pane key="basic" tab="基本信息">
          ...
          <a-form-item label="实例ID" name="instance_id">
            <a-input
              v-model:value="formData.instance_id"
              placeholder="请输入实例ID"
            />
          </a-form-item>
              ...
              <div v-for="(ip, index) in formData.public_ip" :key="index">
                <a-input-group compact>
                  ...
                  <a-button
                    type="link"
                    danger
                    @click="removeIP('public_ip', index)"
                  >
              ...
              <div v-for="(ip, index) in formData.private_ip" :key="index">
                <a-input-group compact>
                  ...
                  <a-button
                    type="link"
                    danger
                    @click="removeIP('private_ip', index)"
                  >
                    <template #icon><DeleteOutlined /></template>
                  </a-button>
                </a-input-group>
        ...
        <a-tab-pane key="config" tab="配置信息">
          ...
          <a-form-item label="可用区">
            <a-input
              v-model:value="formData.configuration.zone_id"
              placeholder="可用区ID"
            />
          </a-form-item>
          ...
          <a-form-item label="VPC ID">
            <a-input
              v-model:value="formData.configuration.vpc_id"
              placeholder="VPC ID"
            />
          </a-form-item>
        </a-tab-pane>
...
const handleProviderChange = (providerId: number) => {
  ...
  if (provider) {
    formData.resource_type = provider.type === 'aliyun' ? 'ecs' : 
                             provider.type === 'aws' ? 'ec2' : 
                             provider.type === 'tencent' ? 'cvm' : 'manual'
...
const handleSubmit = async () => {
  try {
    ...
    const submitData = {
      ...formData,
      expired_at: formData.expired_at ? dayjs(formData.expired_at).format('YYYY-MM-DD HH:mm:ss') : undefined,

...
const primaryIP = computed(() => {
  ...
  // 调试输出
  console.log('🔍 Debug host data:', props.host)
  console.log('🔍 public_ip field:', props.host.public_ip)
  console.log('🔍 private_ip field:', props.host.private_ip)
  ...
  try {
    ...
    console.log('🔍 Parsed publicIPs:', publicIPs)
    // 过滤空字符串，找到第一个有效的公网IP
    const validPublicIP = publicIPs.find(ip => ip && ip.trim() !== '')
    if (validPublicIP) {
      console.log('🔍 Using public IP:', validPublicIP)
      return validPublicIP
    }
  } catch (e) {
    console.log('🔍 Error parsing public_ip:', e)
  ...
  try {
    ...
    console.log('🔍 Parsed privateIPs:', privateIPs)
    // 过滤空字符串，找到第一个有效的私网IP
    const validPrivateIP = privateIPs.find(ip => ip && ip.trim() !== '')
    if (validPrivateIP) {
      console.log('🔍 Using private IP:', validPrivateIP)
      return validPrivateIP
    }
  } catch (e) {
    console.log('🔍 Error parsing private_ip:', e)
    // 如果解析失败，尝试直接使用（但也要检查是否为空）

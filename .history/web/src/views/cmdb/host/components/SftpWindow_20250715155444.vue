...
export default defineComponent({
  ...
  setup() {
    ...
    const currentHost = ref<number | null>(null);
    ...
    const uploadForm = ref({ file: null as File | null, path: currentPath.value });
    ...
    const columns = [
      ...
      },
      {
        ...
        dataIndex: 'mod_time',
        key: 'mod_time',
        sorter: (a: FileItem, b: FileItem) => new Date(a.mod_time).getTime() - new Date(b.mod_time).getTime(),
        width: '25%'
      },
      {
        title: '权限',
    ...
    const getHostIP = (host: Host): string => {
      try {
    ...
    const listFiles = async () => {
      ...
      try {
        ...
        if (data.code === 0) {
          fileList.value = data.data.map((item: any) => ({
            ...item,
            key: item.path
          }));
          updateDirectoryTree();
    ...
    const getPathUntil = (index: number): string => {
    ...
    const createDirectory = async () => {
      ...
      try {
        ...
        if (data.code === 0) {
          ...
          message.error(data.message || '创建目录失败');
        }
      } catch (error) {
    ...
    const confirmDelete = (file: FileItem) => {
      ...
      window.confirm({
        title: `确认删除${fileType}`,
        content: `确定要删除${fileType} "${file.name}" 吗？`,
        okText: '删除',
        okType: 'danger',
        cancelText: '取消',

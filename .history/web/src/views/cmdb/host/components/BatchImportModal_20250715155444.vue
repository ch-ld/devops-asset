...
const handleFileSelect = (file: File) => {
  ...
  if (!isValidType) {
    message.error('只支持Excel(.xlsx)或CSV(.csv)格式文件')
    return false
  }
...
const downloadTemplate = () => {
  // 创建模板数据
  const templateData = [
    ['provider_id', 'instance_id', 'name', 'resource_type', 'region', 'username', 'password', 'public_ip', 'private_ip', 'configuration', 'os', 'status', 'expired_at', 'remark'],
    ['1', 'i-example123', '示例主机', 'ecs', 'cn-hangzhou', 'root', 'password123', '["1.2.3.4"]', '["192.168.1.10"]', '{"cpu_cores":2,"memory_size":4}', 'CentOS 7', 'running', '2024-12-31 23:59:59', '示例备注']
  ]

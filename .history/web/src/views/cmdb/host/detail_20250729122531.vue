<template>
  <div class="host-detail-page">
    ...
    <!-- 现代化组件 -->
    <modern-s-s-h-terminal v-model:visible="modernTerminalVisible" :host="host" />
    <sftp-manager v-model:visible="modernSftpVisible" :host="host" />
  </div>
</template>
...
<script setup>
  ...
  import ModernSSHTerminal from './components/ModernSSHTerminal.vue'
  ...
  const modernTerminalVisible = ref(false)
  const modernSftpVisible = ref(false)
  ...
  const handleMenuClick = async (command) => {
    switch (command) {
      case 'ssh':
        // 使用现代化SSH终端
        modernTerminalVisible.value = true
        break
      case 'sftp':
        // 使用现代化文件管理器
        modernSftpVisible.value = true
        break
      case 'sync':

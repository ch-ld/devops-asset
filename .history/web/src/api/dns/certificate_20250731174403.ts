...
export const certificateApi = {
  ...
  list: (params: CertificateListParams) => {
    return request.get<{
      items: Certificate[]
      total: number
    }>('/api/v1/dns/certificates', { params })
  },

  // 获取证书详情
  get: (id: number) => {
    return request.get<Certificate>(`/api/v1/dns/certificates/${id}`)
  },

  // 申请证书
  issue: (data: CertificateCreateRequest) => {
    return request.post<Certificate>('/api/v1/dns/certificates', data)
  },

  // 续期证书
  renew: (id: number) => {
    return request.post<Certificate>(`/api/v1/dns/certificates/${id}/renew`)
  },

  // 吊销证书
  revoke: (id: number, reason?: string) => {
    return request.post(`/api/v1/dns/certificates/${id}/revoke`, { reason })
  },

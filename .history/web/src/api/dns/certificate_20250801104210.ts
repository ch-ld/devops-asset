...
export const certificateApi = {
  ...
  deploy: (id: number, targets: string[]) => {
    return request.post({ url: `/api/v1/dns/certificates/${id}/deploy`, data: { targets } })
  },
  ...
  // 删除证书
  delete: (id: number) => {
    return request.del({ url: `/api/v1/dns/certificates/${id}` })
  },

  // 批量删除证书
  batchDelete: (ids: number[]) => {
    return request.del({ url: '/api/v1/dns/certificates/batch', data: { ids } })
  }
}

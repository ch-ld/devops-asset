...
export const dnsProviderApi = {
  ...
  testConnection: (data: any) => {
    return request.post<{ success: boolean; message: string }>('/api/v1/dns/providers/test-connection', data)
  },
  ...
  batchTest: (ids: number[]) => {
    return request.post({ url: '/api/v1/dns/providers/batch-test', data: { ids } })
  },

...
export const domainApi = {
   ...
   * 下载导入模板
   */
  downloadTemplate: (format: 'csv' | 'excel'): Promise<Blob> => {
    const url = format === 'csv'
      ? '/api/v1/dns/domains/template/csv'
      : '/api/v1/dns/domains/template/excel'
  ...
  import: (data: {
    ...
    return api.post({ url: '/api/v1/dns/import-export/domains/import', data })
  },
  ...
  export: (params: {
    ...
    const url = format === 'csv'
      ? '/api/v1/dns/domains/export/csv'
      : '/api/v1/dns/domains/export/excel'
    return api.get({ url, params: params.filters, responseType: 'blob' })
  },
  ...
  whois: (id: number): Promise<{
    ...
    return api.get({ url: `/api/v1/dns/domains/${id}/whois` })
  },
...
// 兼容性导出
export const getDomains = domainApi.list

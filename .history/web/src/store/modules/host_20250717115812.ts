...
export const useHostStore = defineStore('hostStore', () => {
  ...
  const fetchHosts = async (params?: {
    ...
    try {
      const response = (await hostApi.getHostList(
        params
      )) as unknown as ApiResponse<HostListResponse>
      ...
      // 设置错误状态
      error.value = err instanceof Error ? err : new Error('获取主机列表失败')
      // 确保即使API调用失败，hostList也是一个空数组而不是undefined
      hostList.value = []
  ...
  const batchMoveHosts = async (ids: number[], groupId?: number) => {
    isSubmitting.value = true
    try {
      await hostApi.batchMoveHosts({ ids, group_id: groupId })
      await fetchHosts()
    } catch (error) {
  ...
  const loadHostAlerts = async (days = 7) => {
    isLoading.value = true
    try {
      const response = (await hostApi.getHostAlerts(days)) as unknown as ApiResponse<HostAlert[]>
      hostAlerts.value = response.data
    } catch (error) {
      console.error('Failed to load host alerts:', error)
      // 确保即使API调用失败，hostAlerts也是一个空数组而不是undefined
      hostAlerts.value = []
      // 不再向上抛出错误，避免阻断UI渲染
      // throw error
    } finally {

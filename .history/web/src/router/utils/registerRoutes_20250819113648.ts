...
function convertRouteComponent(
  ...
  // 针对证书详情的路径歧义处理：避免 `certs/:id` 抢占 `certs/create-advanced`
  // 将参数限制为数字，仅匹配如 /dns/certs/123
  if (
    route.name === 'DnsCertDetail' &&
    typeof route.path === 'string' &&
    route.path.includes(':id')
  ) {
    converted.path = route.path.replace(':id', ':id(\\d+)')
  }
...
function handleLayoutRoute(
  ...
  // 子路由路径（处理证书详情的参数限制）
  let childPath = route.path as string
  if (route.name === 'DnsCertDetail' && typeof childPath === 'string' && childPath.includes(':id')) {
    childPath = childPath.replace(':id', ':id(\\d+)')
  }

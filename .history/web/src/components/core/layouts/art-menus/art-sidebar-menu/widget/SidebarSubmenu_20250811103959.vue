...
<script setup lang="ts">
  ...
  const handleMenuClick = async (item: AppRouteRecord) => {
    try {
      ...
      // 如果路由尚未注册，则进行多次重试等待动态路由完成注册
      if (!router.hasRoute(routeName)) {
        console.warn(`路由 ${routeName} 未注册，等待动态注册完成...`)
        ...
        const MAX_WAIT_COUNT = 15 // 增加重试次数到15次
        const DELAY = 300 // 增加间隔到300ms

        let count = 0
        while (count < MAX_WAIT_COUNT && !router.hasRoute(routeName)) {
        ...
        if (!router.hasRoute(routeName)) {
          ElMessage.warning('页面加载超时，请刷新页面后重试')
          return
        }
      }

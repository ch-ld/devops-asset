...
### 5.2 状态管理（Pinia）
- `dnsStore`：domains、certificates、records、loading、error。

### 5.3 关键组件
| 组件 | 说明 |
|------|------|
| `DomainTable.vue` | 可拖拽分组，支持列自定义 |
| `CertRequestDialog.vue` | 向导形式，选择域名 → 填写 SAN → 选择部署主机 |
| `RecordDiffViewer.vue` | Monoco-diff 展示云端 vs 本地差异 |

### 5.4 交互细节
1. 所有删除操作二次确认（弹窗输入域名验证）。
2. 高危变更（NS、CNAME root）需输入工单号备注。
3. 列表支持本地搜索 & Provider 筛选。

### 5.5 样式规范
- 颜色继承平台主题；域名状态颜色与主机状态保持一致（正常=green / 将过期=orange / 已过期=red）。
...
> 结合 certd、allinssl 等开源实践，聚焦以下 6 项增强：
...
### 15.3 K8s Ingress / Istio 自动部署
- **Deployer 抽象**：`deployer/driver.go`，提供 `Deploy(cert *Certificate, target Target) error`。
- 内置实现：
  1. `SFTPDeployer`（已存在）
  2. `K8sIngressDeployer`：Patch TLS Secret → Reload Ingress
  3. `IstioDeployer`：更新 `Gateway` 资源 → Rollout Rel.
- UI 在证书向导 Step3 中可选择 "Kubernetes 集群"。
...
| P3 | K8s/Istio Deployer + HTTPS Monitor + 扩展通知 | +15d |

...
// jsonWebSocketWriter wraps WebSocket connection to send JSON formatted messages
type jsonWebSocketWriter struct {
	conn *websocket.Conn
}

// Write implements io.Writer interface, converting raw data to JSON format
func (w *jsonWebSocketWriter) Write(p []byte) (n int, err error) {
	if len(p) == 0 {
		return 0, nil
	}

	// Create JSON message for SSH output
...
func (st *SshTerminal) getWsReadWriter() (io.Reader, io.Writer, error) {
	...
	wsReader := func(p []byte) (n int, err error) {
		for {
			...
			if msgType == websocket.BinaryMessage {
				// Handle resize commands (sent as JSON in binary message)
				data := make([]byte, 512)
				...
				var size termSize
				if err := json.Unmarshal(data[:n], &size); err == nil {
					st.resizeEvent <- size
					continue
				}
				// If not resize command, treat as regular input
				copy(p, data[:n])
				return n, nil
			}
	...
	// Custom WebSocket writer that sends raw SSH output as text
	wsWriter := &rawWebSocketWriter{
		conn: st.wsConn,
	}

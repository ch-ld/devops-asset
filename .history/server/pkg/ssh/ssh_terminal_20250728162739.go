...
// rawWebSocketWriter wraps WebSocket connection to send raw SSH data
type rawWebSocketWriter struct {
	conn *websocket.Conn
}

// Write implements io.Writer interface, sending raw SSH data as text
func (w *rawWebSocketWriter) Write(p []byte) (n int, err error) {
	if len(p) == 0 {
		return 0, nil
	}

	// Send raw SSH output as text message

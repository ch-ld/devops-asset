...
func (h *CertificateHandler) ValidateCSR(c *gin.Context) {
	...
	if err != nil {
		response.ReturnError(c, response.INTERNAL_SERVER_ERROR, "CSR验证失败: "+err.Error())
		return
	}

	response.ReturnSuccess(c, result)
}
...
func (h *CertificateHandler) UploadCSRFile(c *gin.Context) {
	...
	src, err := file.Open()
	if err != nil {
		response.ReturnError(c, response.INTERNAL_SERVER_ERROR, "打开文件失败: "+err.Error())
		return
	}
	defer src.Close()
	...
	if err != nil {
		response.ReturnError(c, response.INTERNAL_SERVER_ERROR, "处理CSR文件失败: "+err.Error())
		return
	}
...
func (h *CertificateHandler) IssueCertificateWithCSR(c *gin.Context) {
	...
	if err != nil {
		response.ReturnError(c, response.INTERNAL_SERVER_ERROR, "证书申请失败: "+err.Error())
		return
	}

	response.ReturnSuccess(c, certificate)
}

...
// @Description DNS提供商列表响应
type ProviderListResponse struct {
	Total int                 `json:"total" example:"10" comment:"总数量"`
	Items []*ProviderResponse `json:"items" comment:"提供商列表"`
}
...
// @Summary 创建DNS提供商
// @Description 创建新的DNS提供商配置
// @Tags DNS提供商管理
// @Accept json
// @Produce json
// @Param request body CreateProviderRequest true "DNS提供商创建请求"
...
func (h *ProviderHandler) CreateProvider(c *gin.Context) {
	...
	if err := h.providerService.CreateProvider(provider, userID.(uint), c.ClientIP()); err != nil {
		response.Error(c, http.StatusInternalServerError, "创建DNS提供商失败: "+err.Error())
		return
	}
...
// @Summary 更新DNS提供商
// @Description 更新指定ID的DNS提供商信息
// @Tags DNS提供商管理
// @Accept json
// @Produce json
// @Param id path int true "提供商ID"
// @Param request body UpdateProviderRequest true "DNS提供商更新请求"
...
func (h *ProviderHandler) UpdateProvider(c *gin.Context) {
	...
	if err != nil {
		response.Error(c, http.StatusBadRequest, "无效的提供商ID")
		return
	}
	...
	if err := h.providerService.UpdateProvider(provider, userID.(uint), c.ClientIP()); err != nil {
		response.Error(c, http.StatusInternalServerError, "更新DNS提供商失败: "+err.Error())
		return
	}
	...
	if err != nil {
		response.Error(c, http.StatusInternalServerError, "获取更新后的提供商信息失败: "+err.Error())
		return
	}
...
func (h *ProviderHandler) DeleteProvider(c *gin.Context) {
	...
	if err := h.providerService.DeleteProvider(uint(id), userID.(uint), c.ClientIP()); err != nil {
		response.Error(c, http.StatusInternalServerError, "删除DNS提供商失败: "+err.Error())
		return
	}
...
func (h *ProviderHandler) GetProvider(c *gin.Context) {
	...
	if err != nil {
		response.Error(c, http.StatusNotFound, "DNS提供商不存在: "+err.Error())
		return
	}
...
// @Summary 获取DNS提供商列表
// @Description 获取DNS提供商列表，支持分页和筛选
// @Tags DNS提供商管理
// @Produce json
...
func (h *ProviderHandler) ListProviders(c *gin.Context) {
	...
	if err != nil {
		response.Error(c, http.StatusInternalServerError, "获取DNS提供商列表失败: "+err.Error())
		return
	}
...
func (h *ProviderHandler) TestProvider(c *gin.Context) {
	...
	if err != nil {
		response.Error(c, http.StatusInternalServerError, "测试DNS提供商连接失败: "+err.Error())
		return
	}
	...
	response.Success(c, testResp)
}

// SetDefaultProvider 设置默认DNS提供商
// @Summary 设置默认DNS提供商
// @Description 设置指定的DNS提供商为默认提供商
// @Tags DNS提供商管理
// @Produce json
// @Param id path int true "提供商ID"
...
func (h *ProviderHandler) SetDefaultProvider(c *gin.Context) {
	...
	if err := h.providerService.SetDefaultProvider(uint(id), userID.(uint), c.ClientIP()); err != nil {
		response.Error(c, http.StatusInternalServerError, "设置默认DNS提供商失败: "+err.Error())
		return
	}

...
func (h *RecordImportExportHandler) ImportRecordsFromJSON(c *gin.Context) {
	...
	for i, item := range req.Data {
		...
		if v := toPtrInt(item["priority"]); v != nil { rec.Priority = v }
		if v := toPtrInt(item["weight"]); v != nil { rec.Weight = v }
		if v := toPtrInt(item["port"]); v != nil { rec.Port = v }
...
func (h *RecordImportExportHandler) ExportRecords(c *gin.Context) {
	...
	for _, r := range records {
		...
		for _, f := range fields {
			switch f {
			case "name": row = append(row, r.Name)
			case "type": row = append(row, r.Type)
			case "value": row = append(row, r.Value)
			case "ttl": row = append(row, strconv.Itoa(r.TTL))
			case "priority": if r.Priority!=nil { row = append(row, strconv.Itoa(*r.Priority)) } else { row = append(row, "") }
			case "weight": if r.Weight!=nil { row = append(row, strconv.Itoa(*r.Weight)) } else { row = append(row, "") }
			case "port": if r.Port!=nil { row = append(row, strconv.Itoa(*r.Port)) } else { row = append(row, "") }
			case "remark": row = append(row, r.Remark)
			default: row = append(row, "")
			}
		}
	}
	w.Flush()
...
func toUint(v interface{}) uint {
	...
	case int:
		return uint(t)
	case string:
		if n, err := strconv.Atoi(t); err == nil { return uint(n) }
	}
	return 0
}

...
func (h *RecordHandler) CreateRecord(c *gin.Context) {
	...
	record := &dns.Record{
		...
		Port:       &req.Port,
		Status:     "active",
		SyncStatus: "synced", // 设置初始同步状态为已同步
		Remark:     req.Remark,
...
// @Summary 调试DNS记录删除
// @Description 调试DNS记录删除过程，检查CloudRecordID和同步状态
// @Tags DNS记录
// @Accept json
// @Produce json
...
// @Router /api/v1/dns/records/{id}/debug [get]
func (h *RecordHandler) DebugRecord(c *gin.Context) {
	// 获取记录ID
	idStr := c.Param("id")
	id, err := strconv.ParseUint(idStr, 10, 32)
	if err != nil {
	...
	debugInfo := map[string]interface{}{
		...
		// 格式化的信息
		"fallback_id":  fmt.Sprintf("%s:%s", record.Name, record.Type),
		"has_cloud_id": record.CloudRecordID != "",
		"can_delete":   record.CloudRecordID != "" || (record.Name != "" && record.Type != ""),

		// 预测删除行为
		"delete_strategy": func() string {
			if record.CloudRecordID != "" {
				return fmt.Sprintf("将使用CloudRecordID '%s' 删除", record.CloudRecordID)
			} else if record.Name != "" && record.Type != "" {
				return fmt.Sprintf("将使用name:type格式 '%s:%s' 删除", record.Name, record.Type)
			} else {
				return "无法删除：缺少必要信息"
			}
		}(),
	...
	response.ReturnData(c, debugInfo)
}
...
func (h *RecordHandler) SyncRecord(c *gin.Context) {
	...
	if err := h.recordService.SyncRecord(uint(id), userID.(uint), c.ClientIP()); err != nil {
		...
		case strings.Contains(err.Error(), "provider not found"):
			errorMsg = "DNS提供商配置不存在，请检查提供商设置"
		case strings.Contains(err.Error(), "record not found"):
			errorMsg = "DNS记录不存在"
		case strings.Contains(err.Error(), "authentication failed"):
			errorMsg = "DNS提供商认证失败，请检查API密钥配置"
		case strings.Contains(err.Error(), "rate limit"):
			errorMsg = "API调用频率超限，请稍后重试"
		case strings.Contains(err.Error(), "network"):
			errorMsg = "网络连接失败，请检查网络设置"
		case strings.Contains(err.Error(), "empty response"):
			errorMsg = "DNS提供商返回空内容，可能是域名未在该提供商托管"
		case strings.Contains(err.Error(), "domain not managed"):
			errorMsg = "该域名未在DNS提供商托管，无法同步"
		default:
			errorMsg = fmt.Sprintf("同步DNS记录失败: %s", err.Error())
		}

		response.ReturnError(c, response.INTERNAL, errorMsg)
		return
	}
...
func (h *RecordHandler) BatchSyncRecords(c *gin.Context) {
	...
	if err := h.recordService.BatchSyncRecords(req.IDs, userID.(uint), c.ClientIP()); err != nil {
		...
		case strings.Contains(err.Error(), "network"):
			errorMsg = "网络连接失败，请检查网络设置"
		case strings.Contains(err.Error(), "partial failure"):
			errorMsg = "部分记录同步失败，请检查错误详情"
		default:
			errorMsg = fmt.Sprintf("批量同步DNS记录失败: %s", err.Error())
		}
...
// DebugSyncStatus 调试同步状态 - 仅用于开发调试
// @Summary 调试同步状态
// @Description 查看DNS记录的实际同步状态数据，仅用于开发调试
// @Tags DNS记录
// @Accept json
// @Produce json
// @Success 200 {object} response.Response
// @Router /api/v1/dns/records/debug/sync-status [get]
func (h *RecordHandler) DebugSyncStatus(c *gin.Context) {
	// 获取租户ID
	tenantID, exists := c.Get("tenant_id")
	if !exists {
		response.ReturnError(c, response.UNAUTHENTICATED, "用户未登录")
	...
	// 获取调试信息
	debugInfo, err := h.recordService.GetDebugSyncStatus(tenantID.(uint))
	if err != nil {
		response.ReturnError(c, response.INTERNAL, "获取调试信息失败: "+err.Error())
		return
	}
...
// FixSyncStatus 修复同步状态 - 仅用于开发调试
// @Summary 修复同步状态
// @Description 修复DNS记录的同步状态，将错误状态重置为正常
// @Tags DNS记录
// @Accept json
// @Produce json
// @Success 200 {object} response.Response
// @Router /api/v1/dns/records/debug/fix-sync-status [post]
func (h *RecordHandler) FixSyncStatus(c *gin.Context) {
	// 获取租户ID
	tenantID, exists := c.Get("tenant_id")
	if !exists {
		response.ReturnError(c, response.UNAUTHENTICATED, "用户未登录")
	...
	// 修复同步状态
	result, err := h.recordService.FixSyncStatus(tenantID.(uint))
	if err != nil {
		response.ReturnError(c, response.INTERNAL, "修复同步状态失败: "+err.Error())
		return
	}

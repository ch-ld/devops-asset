...
type IssueCertificateRequest struct {
	DomainID          uint     `json:"domain_id" example:"1" comment:"域名ID（可选，用于关联域名）"`
	...
	KeyType           string   `json:"key_type" example:"RSA2048" comment:"密钥类型(RSA1024/RSA2048/RSA3072/RSA4096/EC256/EC384)"`
	ValidDays         int      `json:"valid_days" example:"90" comment:"有效天数"`
	AutoRenew         bool     `json:"auto_renew" example:"true" comment:"自动续期"`
	RenewDays         int      `json:"renew_days" example:"35" comment:"提前续期天数"`
	ChallengeType     string   `json:"challenge_type" example:"dns" comment:"验证方式(dns/cname/http)"`
	ScheduleTime      string   `json:"schedule_time" example:"02:00" comment:"定时触发时间"`
	NotificationType  string   `json:"notification_type" example:"default" comment:"通知类型(default/custom)"`
	NotificationEmail string   `json:"notification_email" example:"notify@example.com" comment:"自定义通知邮箱"`
...
func (h *CertificateHandler) ListCertificates(c *gin.Context) {
	...
	for i, cert := range result.List {
		// 从DomainName中提取SAN域名列表（如果有的话）
		var subjectAltNames []string
		if cert.DomainName != "" {
			// 简单处理：如果DomainName包含多个域名，用逗号分隔
			if strings.Contains(cert.DomainName, ",") {
				subjectAltNames = strings.Split(cert.DomainName, ",")
				for j := range subjectAltNames {
					subjectAltNames[j] = strings.TrimSpace(subjectAltNames[j])
				}
			} else {
				subjectAltNames = []string{cert.DomainName}
			}
		}
		...
		certificateResponses[i] = &CertificateResponse{
			...
			CAType:          cert.CAType,
			Status:          cert.Status,
			SerialNumber:    "", // 暂时为空，后续可从数据库获取
			Fingerprint:     "", // 暂时为空，后续可从数据库获取
			IssuedAt:        cert.IssuedAt,
			ExpiresAt:       cert.ExpiresAt,
			AutoRenew:       cert.AutoRenew,
			RenewDays:       30,  // 默认值，后续可从数据库获取
			LastRenewAt:     nil, // 暂时为空，后续可从数据库获取
			Remark:          cert.Remark,
			TenantID:        tenantID.(uint),
...
func (h *CertificateHandler) DeleteCertificate(c *gin.Context) {
	...
	// 删除证书
	err = h.certificateService.DeleteCertificate(c.Request.Context(), uint(id), tenantID.(uint), userID.(uint))
	if err != nil {
		response.ReturnError(c, response.INTERNAL, "删除证书失败: "+err.Error())
		return
	}
...
// BatchDeleteCertificatesRequest 批量删除证书请求
...
func (h *CertificateHandler) BatchDeleteCertificates(c *gin.Context) {
	var req BatchDeleteCertificatesRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		response.ReturnError(c, response.INVALID_ARGUMENT, "请求参数错误: "+err.Error())
		return
	}

...
// @Summary 获取域名分组统计信息
// @Description 获取域名分组的统计信息
// @Tags DNS域名分组
// @Produce json
// @Success 200 {object} response.Response{data=map[string]interface{}} "获取成功"
// @Failure 500 {object} response.Response "服务器内部错误"
// @Router /api/dns/domain-groups/statistics [get]
func (h *DomainGroupHandler) GetDomainGroupStatistics(c *gin.Context) {
	// 获取租户信息
	tenantID, exists := c.Get("tenant_id")
	if !exists {
		response.ReturnError(c, response.UNAUTHENTICATED, "租户信息缺失")
	...
	// 获取域名分组统计信息
	stats, err := h.domainGroupService.GetDomainGroupStatistics(tenantID.(uint))
	if err != nil {
		response.ReturnError(c, response.INTERNAL, "获取域名分组统计信息失败: "+err.Error())
		return
	}

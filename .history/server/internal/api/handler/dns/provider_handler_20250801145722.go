...
// TestConnectionRequest 临时测试连接请求体结构体
type TestConnectionRequest struct {
	Name        string            `json:"name" binding:"required" example:"测试提供商" comment:"提供商名称"`
	Type        string            `json:"type" binding:"required" example:"aliyun" comment:"提供商类型"`
	Credentials map[string]string `json:"credentials" binding:"required" comment:"凭证信息"`
	Remark      string            `json:"remark" example:"测试连接" comment:"备注"`
}
...
func (h *ProviderHandler) TestConnection(c *gin.Context) {
	...
	// 临时测试连接
	result, err := h.providerService.TestConnectionTemporary(req.Type, req.Name, req.Credentials)
	if err != nil {
		response.ReturnError(c, response.INTERNAL, "测试DNS提供商连接失败: "+err.Error())
		return
	}

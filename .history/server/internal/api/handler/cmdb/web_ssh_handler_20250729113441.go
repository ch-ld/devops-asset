...
func (h *WebSshHandler) HandleWebSSH(c *gin.Context) {
	...
	if len(decryptedPassword) > 0 {
		log.Printf("🔍 [WebSSH] Password length: %d", len(decryptedPassword))
		log.Printf("🔍 [WebSSH] Encrypted password: %s", host.Password)
		log.Printf("🔍 [WebSSH] Decrypted password: %s", decryptedPassword)
	}

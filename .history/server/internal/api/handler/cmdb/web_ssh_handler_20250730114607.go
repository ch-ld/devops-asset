...
func (h *WebSshHandler) HandleWebSSH(c *gin.Context) {
	...
	if err != nil {
		zap.L().Error("Failed to create SSH session", 
			zap.String("host", host.Name), 
			zap.Error(err))
		wsConn.WriteMessage(websocket.TextMessage, []byte("会话创建失败"))
		wsConn.Close()
		h.updateSSHStatus(uint(hostID), "failed", nil)
		return
	}
	...
	// 启动会话处理
	sshHandler := ssh.NewSSHHandler(wsConn, sshSession)
	if sshHandler == nil {
		zap.L().Error("Failed to create SSH handler", zap.String("host", host.Name))
	...
	go func() {
		...
		if err := sshHandler.LoopRead(); err != nil {

...
// @Description 根据云厂商类型动态获取支持的地域列表
// @Tags 云账号管理
// @Accept json
// @Produce json
...
func (h *ProviderHandler) GetProviderRegions(c *gin.Context) {
	...
	case "alicloud", "tencentcloud", "aws":
		// 创建一个临时的Provider对象用于初始化适配器
		// 注意：这里使用空的凭证，因为获取区域列表通常不需要凭证
		// 但有些云厂商可能需要凭证，这种情况下需要提供默认的凭证或处理错误
		tempProvider := &model.Provider{
			Type:      providerType,
			AccessKey: "", // 某些云厂商获取区域可能不需要凭证
			SecretKey: "",
			Region:    "",
		}
		...
		if err != nil {
			// 如果API调用失败，回退到静态区域列表
			zap.L().Warn("Failed to get regions from cloud provider API, falling back to static list",

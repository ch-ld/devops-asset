...
func NewHostBatchHandler(hostService *svc.HostService) *HostBatchHandler {
...
func (h *HostBatchHandler) HandleBatchChangeStatus(c *gin.Context) {
	...
	batchReq := repoModel.BatchChangeStatusRequest{
		IDs:    req.Ids,
		Status: req.Status,
	}
...
func (h *HostBatchHandler) HandleBatchLifecycle(c *gin.Context) {
	...
	if req.ExpiredAt != nil && *req.ExpiredAt != "" {
		...
		if err != nil {
			response.ReturnError(c, response.INVALID_ARGUMENT, "过期时间格式错误，应为YYYY-MM-DD")
			return
		}
		expiredAt = &t
	}

	batchReq := repoModel.BatchLifecycleRequest{
		IDs:       req.Ids,
		ExpiredAt: expiredAt,
		Status:    req.Status,
		Recycle:   req.Recycle,
...
func (h *HostBatchHandler) HandleBatchSetCustomFields(c *gin.Context) {
	...
	batchReq := repoModel.BatchSetCustomFieldsRequest{
		IDs:         req.Ids,
...
func (h *HostBatchHandler) HandleBatchSSH(c *gin.Context) {
	...
	batchReq := repoModel.BatchSSHRequest{
		IDs:     req.Ids,
		Cmd:     req.Cmd,
		Timeout: req.Timeout,
	}
...
func (h *HostBatchHandler) RegisterRoutes(router *gin.RouterGroup) {
	...
	{
		batchRouter.DELETE("/batch_delete", h.HandleBatchDelete)

...
func (h *HostHandler) CreateHost(c *gin.Context) {
	...
	encryptedPrivateKey := req.PrivateKey
	if req.PrivateKey != "" {
		var err error
...
func (h *HostHandler) parseHostRecord(record []string, headers []string) (*model.Host, error) {
	...
	// provider_id 可以为空或"-"，表示手动添加的主机
	// 如果 resource_type 是 manual，则忽略 provider_id
	if fields["resource_type"] != "manual" && fields["provider_id"] != "" && fields["provider_id"] != "-" {
		providerID, err = strconv.ParseUint(fields["provider_id"], 10, 32)
		if err != nil {
			return nil, fmt.Errorf("provider_id格式错误")
		}

		// 验证 provider_id 是否存在（暂时跳过验证，让数据库外键约束处理）
		// if providerID > 0 {
		// 	if err := h.validateProviderExists(uint(providerID)); err != nil {
		// 		return nil, fmt.Errorf("provider_id不存在: %v", err)
		// 	}
		// }
	}
	...
	var providerType string = "manual" // 默认为手动主机

	// 只有当不是手动主机且 providerID 有效时才设置 provider_id
	if fields["resource_type"] != "manual" && providerID != 0 {
		providerIDUint := uint(providerID)
		providerIDPtr = &providerIDUint
		// 根据 resource_type 设置 provider_type
	...
	if fields["password"] != "" {
		var err error
		encryptedPassword, err = encryptData(fields["password"])
		if err != nil {
			return nil, fmt.Errorf("password encryption failed: %w", err)
		}
	}

	encryptedPrivateKey := fields["private_key"]
	if fields["private_key"] != "" {
		var err error
		encryptedPrivateKey, err = encryptData(fields["private_key"])
		if err != nil {
...
func (h *HostHandler) BatchExportHosts(c *gin.Context) {
	...
	if fieldsParam != "" {
		...
		// 默认字段
		fields = []string{"name", "instance_id", "status", "public_ip", "private_ip", "configuration", "os", "region"}
	}
...
func (h *HostHandler) formatConfiguration(configData datatypes.JSON) string {
	...
	} else if cpuCores, ok := config["cpu_cores"]; ok {
		parts = append(parts, fmt.Sprintf("CPU: %v核", cpuCores))
	}

	// 内存信息 - 兼容多种字段名
	if memory, ok := config["memory"]; ok {
		parts = append(parts, fmt.Sprintf("内存: %vGB", memory))
	} else if memorySize, ok := config["memory_size"]; ok {
		parts = append(parts, fmt.Sprintf("内存: %vGB", memorySize))
	}

	// 磁盘信息 - 兼容多种字段名
	if disk, ok := config["disk"]; ok {
		parts = append(parts, fmt.Sprintf("磁盘: %vGB", disk))
	} else if diskSize, ok := config["disk_size"]; ok {
		parts = append(parts, fmt.Sprintf("磁盘: %vGB", diskSize))
	}
...
// validateProviderExists 验证云账号是否存在
func (h *HostHandler) validateProviderExists(providerID uint) error {
	// 通过 HostService 验证 provider 是否存在
	// 这里我们可以尝试同步主机来验证 provider 是否存在
	// 但更简单的方法是直接查询数据库
	return h.hostSvc.ValidateProvider(providerID)
}

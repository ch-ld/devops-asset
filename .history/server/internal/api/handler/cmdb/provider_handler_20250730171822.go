...
type ProviderHandler struct {
	...
	regionSvc   *svc.RegionService
}

// NewProviderHandler 创建云账号处理器实例
// @Description 创建云账号管理处理器的新实例
// @Param providerSvc 云账号服务实例
// @Param regionSvc 区域服务实例
// @Return *ProviderHandler 云账号处理器实例
func NewProviderHandler(providerSvc *svc.ProviderService, regionSvc *svc.RegionService) *ProviderHandler {
...
// SyncResources 同步云资源
// @Summary 同步云账号资源
// @Description 同步指定云账号的资源到CMDB系统，支持多区域选择
// @Tags 云账号管理
// @Accept json
// @Produce json
// @Param id path int true "云账号ID"
// @Param request body object{regions=[]string,group_id=int} true "同步参数"
...
func (h *ProviderHandler) SyncResources(c *gin.Context) {
	...
	provider, err := h.providerSvc.GetProviderByID(uint(id))
	if err != nil {
		response.ReturnError(c, response.NOT_FOUND, "云账号不存在")
		return
	}

	// 执行多区域云账号主机同步
	result, err := h.providerSvc.SyncResourcesMultiRegion(provider, req.Regions, req.GroupID)
	if err != nil {
...
// ValidateCredentials 验证云账号凭证（通过请求体）
// @Summary 验证云账号凭证
// @Description 验证云账号的访问密钥是否有效，用于添加云账号时的凭证验证
// @Tags 云账号管理
// @Accept json
// @Produce json
// @Param request body object{type=string,access_key=string,secret_key=string,region=string} true "验证参数"
// @Success 200 {object} response.Response{data=object} "验证成功"
// @Failure 400 {object} response.Response "参数错误"
// @Failure 500 {object} response.Response "验证失败"
// @Router /api/v1/cmdb/providers/validate [post]
// @Security BearerAuth
func (h *ProviderHandler) ValidateCredentials(c *gin.Context) {
	...
	if err != nil {
		zap.L().Error("验证凭证失败", zap.Error(err))
		response.ReturnError(c, response.INTERNAL, "验证凭证失败: "+err.Error())
		return
	}

	message := "凭证验证成功"
	if !isValid {
		message = "凭证验证失败，请检查AccessKey、SecretKey和区域是否正确"
	}
...
// ValidateProviderCredentials 验证已存在云账号的凭证
// @Summary 验证已存在云账号的凭证
// @Description 验证指定云账号的访问密钥是否有效
// @Tags 云账号管理
// @Accept json
// @Produce json
// @Param id path int true "云账号ID"
...
func (h *ProviderHandler) ValidateProviderCredentials(c *gin.Context) {
	...
	// 验证凭证
	isValid, err := h.providerSvc.ValidateCredentials(provider.Type, provider.AccessKey, provider.SecretKey, provider.Region)
	if err != nil {
		zap.L().Error("验证凭证失败", zap.Error(err))
		response.ReturnError(c, response.INTERNAL, "验证凭证失败: "+err.Error())
		return
...
// @Summary 获取云账号可用区域
// @Description 获取指定云账号支持的所有区域列表，优先从区域表获取，如无数据则返回默认区域
// @Tags 云账号管理
// @Accept json
// @Produce json
// @Param id path int true "云账号ID"
...
func (h *ProviderHandler) GetRegions(c *gin.Context) {
	...
	// 使用区域服务获取区域列表
	regions, err := h.regionSvc.GetRegionsByProvider(provider.Type)
	if err != nil {
		zap.L().Error("获取区域列表失败", zap.Error(err))
...
// SyncRegions 同步云账号区域数据
// @Summary 同步云账号区域数据
// @Description 从云厂商API同步指定云账号的区域数据到本地数据库
// @Tags 云账号管理
// @Accept json
// @Produce json
// @Param id path int true "云账号ID"
...
// @Failure 404 {object} response.Response "云账号不存在"
// @Failure 500 {object} response.Response "服务器内部错误"
// @Router /api/v1/cmdb/providers/{id}/sync-regions [post]
// @Security BearerAuth
func (h *ProviderHandler) SyncRegions(c *gin.Context) {
	id, err := strconv.Atoi(c.Param("id"))
	if err != nil {
		response.ReturnError(c, response.INVALID_ARGUMENT, "无效的云账号ID")
		return
	...
	// 同步区域数据
	err = h.regionSvc.SyncRegionsFromProvider(provider)
	if err != nil {
		zap.L().Error("同步区域数据失败", zap.Error(err))
		response.ReturnError(c, response.INTERNAL, "同步区域数据失败: "+err.Error())
		return
	}

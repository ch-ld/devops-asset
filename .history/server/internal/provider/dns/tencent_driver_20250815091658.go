...
func (d *TencentDriver) DeleteRecord(ctx context.Context, zoneName, recordID string) error {
	var actualRecordID string = recordID

	// 检查recordID格式，如果是name:type格式，需要先查找真实的RecordId
	if strings.Contains(recordID, ":") {
		parts := strings.Split(recordID, ":")
		if len(parts) == 2 {
			name, recordType := parts[0], parts[1]
			d.logger.Info("删除记录使用name:type格式，先查找真实RecordId",
				zap.String("zone", zoneName),
			...
			actualRecordID = targetRecordID
			d.logger.Info("找到匹配的DNS记录",
				zap.String("zone", zoneName),
				zap.String("found_record_id", actualRecordID))
		}
	}

	recordIDUint, err := strconv.ParseUint(actualRecordID, 10, 64)
	if err != nil {
	...
	if err != nil {
		d.logger.Error("Failed to delete record",
			zap.String("zone", zoneName),
			zap.String("record_id", actualRecordID),
			zap.Error(err))
		return fmt.Errorf("failed to delete record %s: %w", actualRecordID, err)
	}

...
func (d *AliyunDriver) ListZones(ctx context.Context, options *ListOptions) ([]Zone, error) {
	...
	for _, domain := range response.Domains.Domain {
		...
		// 如果有到期时间信息，添加到元数据中
		// 注意：阿里云DescribeDomains API 返回的域名信息中不包含到期时间
		// 要获取域名到期时间，需要调用单独的API或使用域名注册商API
		// 这里先预留结构，后续可以集成域名WHOIS查询
		if zone.Metadata == nil {
			zone.Metadata = make(map[string]interface{})
		}
		zone.Metadata["provider"] = "aliyun"
		zone.Metadata["instance_id"] = domain.InstanceId
	...
	if strings.Contains(recordID, ":") {
		...
		if len(parts) == 2 {
			...
			d.logger.Info("找到匹配的DNS记录",
				zap.String("zone", zoneName),
				zap.String("found_record_id", recordID))
		}
	}

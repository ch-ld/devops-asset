...
type Driver interface {
	...
	ListRecords(ctx context.Context, zoneName string) ([]Record, error)
	...
	BatchCreateRecords(ctx context.Context, zoneName string, records []*Record) ([]*Record, error)

	// BatchUpdateRecords 批量更新DNS记录
	BatchUpdateRecords(ctx context.Context, zoneName string, records []*Record) ([]*Record, error)

	// BatchDeleteRecords 批量删除DNS记录
	BatchDeleteRecords(ctx context.Context, zoneName string, recordIDs []string) error

	// SyncZone 同步整个区域
	SyncZone(ctx context.Context, zoneName string) (*SyncResult, error)

	// CreateTXTChallenge 创建TXT验证记录（用于证书DNS-01验证）
	...
	// GetQuota 获取配额信息
	GetQuota(ctx context.Context) (map[string]interface{}, error)

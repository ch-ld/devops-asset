...
func (s *userService) Register(req *RegisterRequest) error {
	...
	if !config.RegistrationEnabled {
		return response.NewServiceError(response.PERMISSION_DENIED, "注册功能未开启")
	}
	...
	// 创建用户
	userStatus := uint(1) // 默认激活状态
	if config.RegistrationRequireApproval {
		userStatus = 2 // 待审批状态
	}
	...
	user := &system.SystemUser{
		...
		Gender:       req.Gender,
		Status:       userStatus,
		RoleID:       config.RegistrationDefaultRoleID,
		DepartmentID: 1, // 默认部门ID，可以根据需要调整
	}

	if err := tx.Create(user).Error; err != nil {
		tx.Rollback()
		zap.L().Error("注册用户失败", zap.Error(err), zap.String("username", req.Username))
		return response.NewServiceError(response.DATA_LOSS, "注册用户失败")
	}
	...
	statusMsg := "用户注册成功"
	if config.RegistrationRequireApproval {
		statusMsg = "用户注册成功，等待管理员审批"
	}
	zap.L().Info(statusMsg, zap.String("username", req.Username), zap.Uint("user_id", user.ID))
	return nil
}

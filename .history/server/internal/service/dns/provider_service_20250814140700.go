...
func (s *ProviderService) BatchDeleteProviders(ids []uint, actorID uint, clientIP string) error {
	return s.db.Transaction(func(tx *gorm.DB) error {
		if err := s.providerRepo.BatchDelete(ids); err != nil {
			return err
		}
		// 记录删除日志
		for _, id := range ids {
			changeLog := &dns.ChangeLog{
				...
				ResourceID:   id,
				Action:       "delete",
				Description:  "批量删除DNS提供商",
				Status:       "success",
				ClientIP:     clientIP,
				ActorID:      actorID,
			}
			_ = s.changeLogRepo.Create(changeLog)
		}
		return nil
	})
}
...
func (s *ProviderService) SyncProviderDomains(providerID uint, actorID uint, clientIP string) (map[string]interface{}, error) {
	...
	zap.L().Info("解密后的凭证字段",
		zap.String("provider", provider.Name),
		zap.Strings("credential_keys", credKeys))
	...
	for _, zone := range zones {
		domain := &dns.Domain{
			Name:          zone.Name,
			RegistrarID:   &providerID, // 注册商ID（保持兼容性）
			ProviderID:    &providerID, // DNS提供商ID（用于DNS记录同步）
			RegistrarType: provider.Type,
			Status:        "active",
			ExpiresAt:     nil, // 默认为空，下面会尝试从zone元数据中获取
			AutoRenew:     false,
			TenantID:      provider.TenantID,
			CreatedBy:     actorID,
			UpdatedBy:     actorID,
		...
		// 处理域名元数据（如Provider信息、实例ID等）
		if zone.Metadata != nil {
			// 尝试获取域名到期时间（如果Provider提供了这个信息）
			if expirationStr, ok := zone.Metadata["expiration_date"].(string); ok && expirationStr != "" {
				// 尝试解析到期时间
				if expTime, err := time.Parse("2006-01-02", expirationStr); err == nil {
					domain.ExpiresAt = &expTime
				} else if expTime, err := time.Parse(time.RFC3339, expirationStr); err == nil {
					domain.ExpiresAt = &expTime
				}
			}

			// 记录Provider特定的元数据信息用于日志
			if instanceID, ok := zone.Metadata["instance_id"].(string); ok && instanceID != "" {
				zap.L().Debug("同步域名的实例ID",
					zap.String("domain", zone.Name),
					zap.String("instance_id", instanceID))
			}
		}

		// 如果新建同名域名，CreatedBy 字段也需要
		domain.CreatedBy = actorID
		if err := s.domainRepo.UpsertByName(domain); err != nil {
			zap.L().Error("同步域名失败",
				zap.String("domain", zone.Name),
				zap.Error(err))
			errorCount++
			errors = append(errors, fmt.Sprintf("域名 %s 同步失败: %v", zone.Name, err))
			continue
		}
		syncedCount++
	}
...
func (s *ProviderService) BatchTestProviders(ids []uint, actorID uint, clientIP string) (map[string]interface{}, error) {
	...
	for _, id := range ids {
		...
		creds, err := s.decryptCredentials(provider)
		if err != nil {
			failed++
			results = append(results, map[string]interface{}{
				"provider_id": id,
		...
		_ = s.providerRepo.UpdateTestResult(provider.ID, testStatus, errMsg)
...
func (s *ProviderService) GetProviderStatistics(tenantID uint) (map[string]interface{}, error) {
	stats := make(map[string]interface{})

	// 总数统计
	total, err := s.providerRepo.CountByTenantID(tenantID)
	if err != nil {
		return nil, fmt.Errorf("统计DNS提供商总数失败: %w", err)
	}
	stats["total"] = total

	// 状态统计（按租户过滤）
	statusCounts, err := s.providerRepo.CountByStatusAndTenant(tenantID)
	if err != nil {
		return nil, fmt.Errorf("统计DNS提供商状态失败: %w", err)
	}
	stats["by_status"] = statusCounts

	// 类型统计（按租户过滤）
	typeCounts, err := s.providerRepo.CountByTypeAndTenant(tenantID)
	if err != nil {
		return nil, fmt.Errorf("统计DNS提供商类型失败: %w", err)
	}

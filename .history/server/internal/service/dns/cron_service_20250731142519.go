...
import (
	...
	"api-server/internal/repository/dns"
...
type CronService struct {
	db                 *gorm.DB
	cron               *cron.Cron
	domainRepo         *dns.DomainRepository
	certificateRepo    *dns.CertificateRepository
	recordRepo         *dns.RecordRepository
	providerRepo       *dns.ProviderRepository
...
func (s *CronService) dnsZoneSync() {
	...
	for _, provider := range providers {
		// 获取该提供商下的所有域名
		domains, err := s.domainRepo.FindByProviderID(provider.ID)
		if err != nil {
			zap.L().Error("Failed to find domains for provider",

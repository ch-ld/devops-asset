...
func (s *CertificateService) IssueCertificate(ctx context.Context, req *CertificateRequest, userID uint, clientIP string) (*CertificateInfo, error) {
	...
	cert := &dns.Certificate{
		...
		KeyType:      req.KeyType,
		ValidFrom:    time.Now(),
		ValidTo:      time.Now().AddDate(0, 0, 90), // Let's Encrypt证书有效期90天
	...
	// 加密存储证书和私钥
	cert.CertificateEnc, err = s.encryptData(certificates.Certificate)
	if err != nil {
		return nil, fmt.Errorf("failed to encrypt certificate: %w", err)
	}

	cert.PrivateKeyEnc, err = s.encryptData(certificates.PrivateKey)
	if err != nil {
		return nil, fmt.Errorf("failed to encrypt private key: %w", err)
	}

	if certificates.IssuerCertificate != nil {
		cert.CaCertificateEnc, err = s.encryptData(certificates.IssuerCertificate)
		if err != nil {

...
func (s *CronService) dnsZoneSync() {
	...
	for _, provider := range providers {
		// 获取该提供商下的所有域名
		domains, findErr := s.domainRepo.FindByProviderID(provider.ID)
		if findErr != nil {
			zap.L().Error("Failed to find domains by provider", zap.Uint("provider_id", provider.ID), zap.Error(findErr))
			continue
		}

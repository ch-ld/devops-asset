...
func (s *RecordService) UpdateRecord(record *dns.Record, actorID uint, clientIP string) error {
	...
	// 继承未传入的关键字段（DomainID/ProviderID/CloudRecordID/TenantID）
	if record.DomainID == 0 {
		record.DomainID = oldRecord.DomainID
	}
	if record.ProviderID == 0 {
		record.ProviderID = oldRecord.ProviderID
	}
	if record.CloudRecordID == "" {
		record.CloudRecordID = oldRecord.CloudRecordID
	}
	if record.TenantID == 0 {
		record.TenantID = oldRecord.TenantID
	}

	// 若未提供 TTL，则沿用旧值，避免置 0 导致校验失败
	if record.TTL == 0 {
...
// CountRecordsByDomainIDs 批量获取域名的记录数量
func (s *RecordService) CountRecordsByDomainIDs(domainIDs []uint, tenantID uint) (map[uint]int64, error) {
	return s.recordRepo.CountByDomainIDs(domainIDs, tenantID)
}

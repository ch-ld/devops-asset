...
func (s *CertificateService) IssueCertificateWithCSR(ctx context.Context, req *CSRUploadRequest, tenantID, actorID uint) (*dns.Certificate, error) {
	...
	certificate := &dns.Certificate{
		...
		Domains:       req.Domains,
		Email:         req.Email,
		KeyType:       csrValidation.KeyAlgorithm,
		KeySize:       csrValidation.KeySize,
...
// processCSRCertificate 处理CSR证书申请
func (s *CertificateService) processCSRCertificate(ctx context.Context, cert *dns.Certificate, csr *x509.CertificateRequest, provider *dns.Provider) {
	...
	client, err := lego.NewClient(config)
	if err != nil {
		s.logger.Error("Failed to create ACME client", zap.Error(err))
		s.certificateRepo.UpdateStatus(cert.ID, "failed", fmt.Sprintf("创建ACME客户端失败: %v", err))
		return
	}
	...
	if err != nil {
		s.logger.Error("Failed to create DNS provider", zap.Error(err))
		s.certificateRepo.UpdateStatus(cert.ID, "failed", fmt.Sprintf("创建DNS提供商失败: %v", err))
		return
	}

	if err := client.Challenge.SetDNS01Provider(dnsProvider); err != nil {
		s.logger.Error("Failed to set DNS provider", zap.Error(err))
		s.certificateRepo.UpdateStatus(cert.ID, "failed", fmt.Sprintf("设置DNS提供商失败: %v", err))
		return
	}
	...
	if user.Registration == nil {
		reg, err := client.Registration.Register(registration.RegisterOptions{TermsOfServiceAgreed: true})
		if err != nil {
			s.logger.Error("Failed to register ACME user", zap.Error(err))
			s.certificateRepo.UpdateStatus(cert.ID, "failed", fmt.Sprintf("ACME用户注册失败: %v", err))
			return
		}
		user.Registration = reg
	}
	...
	// 使用CSR申请证书
	csrDER := csr.Raw
	certResource, err := client.Certificate.ObtainForCSR(certificate.ObtainForCSRRequest{
		CSR:                   csrDER,
		NotBefore:             time.Now(),
		NotAfter:              time.Now().AddDate(0, 0, cert.ValidDays),
		Bundle:                true,
		PreferredChain:        "",
		AlwaysDeactivateAuths: false,
	})

	if err != nil {
		s.logger.Error("Failed to obtain certificate with CSR", zap.Error(err))
		s.certificateRepo.UpdateStatus(cert.ID, "failed", fmt.Sprintf("证书申请失败: %v", err))
		return
	}

	// 加密并保存证书
	encryptedCert, err := s.encryptData(certResource.Certificate)
	if err != nil {
		s.logger.Error("Failed to encrypt certificate", zap.Error(err))
		s.certificateRepo.UpdateStatus(cert.ID, "failed", fmt.Sprintf("证书加密失败: %v", err))
		return
	}

	var encryptedChain string
	if len(certResource.IssuerCertificate) > 0 {
		encrypted, err := s.encryptData(certResource.IssuerCertificate)
		if err != nil {
			s.logger.Error("Failed to encrypt certificate chain", zap.Error(err))
		} else {
			encryptedChain = string(encrypted)
		}
	}

	// 解析证书信息
	pemBlock, _ := pem.Decode(certResource.Certificate)
	if pemBlock != nil {
	...
	updates := map[string]interface{}{
		"status":           "issued",
		"certificate_enc":  string(encryptedCert),
		"chain_enc":        encryptedChain,
	...
	if err := s.certificateRepo.UpdateFields(cert.ID, updates); err != nil {
		s.logger.Error("Failed to update certificate", zap.Error(err))
		return
	}

	// 自动部署到指定主机
	if len(cert.DeployHosts) > 0 {
		s.deployToHosts(ctx, cert.ID, cert.DeployHosts)
	}

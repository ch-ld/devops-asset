...
// calculateDomainCounts 递归计算分组的域名数量
func (s *DomainGroupService) calculateDomainCounts(groups []*dns.DomainGroup, domainCounts map[uint]int64) {
	for i := range groups {
		group := groups[i]

		// 设置直接归属的域名数量
		if count, exists := domainCounts[group.ID]; exists {
			group.DomainCount = int(count)
		} else {
			group.DomainCount = 0
		}
		...
		if len(group.Children) > 0 {
			s.calculateDomainCounts(group.Children, domainCounts)

			// 计算子分组的域名总数
			var childrenTotalDomainCount int
			for _, child := range group.Children {
				childrenTotalDomainCount += child.TotalDomainCount
			}

			// 当前分组的总域名数量 = 直接归属的域名数量 + 子分组的总域名数量
			group.TotalDomainCount = group.DomainCount + childrenTotalDomainCount
		} else {
			// 没有子分组，总域名数量就是直接归属的域名数量
			group.TotalDomainCount = group.DomainCount
		}
	}
}

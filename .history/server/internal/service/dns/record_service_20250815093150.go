...
func (s *RecordService) CreateRecord(record *dns.Record, actorID uint, clientIP string) error {
	...
	if record.ProviderID == 0 {
		...
		if domain.RegistrarID != nil {
			record.ProviderID = *domain.RegistrarID
		} else {
			// 使用默认提供商
			defaultProvider, err := s.providerRepo.FindDefault(domain.TenantID)
			if err != nil {
				if errors.Is(err, gorm.ErrRecordNotFound) {
					return errors.New("没有可用的DNS提供商，请先配置DNS提供商")
				}
				return fmt.Errorf("获取默认DNS提供商失败: %w", err)
			}
			record.ProviderID = defaultProvider.ID
		}
	}
...
func (s *RecordService) DeleteRecord(id uint, actorID uint, clientIP string) error {
	...
	if err != nil {
		...
		// 先尝试从云厂商删除记录（同步操作，确保删除成功）
	var syncError error
	zap.L().Info("开始删除DNS记录", 
		...
		zap.String("cloud_record_id", record.CloudRecordID))
		
	if record.CloudRecordID != "" || (record.Name != "" && record.Type != "") {
		zap.L().Info("调用云厂商删除接口", 
			zap.String("cloud_record_id", record.CloudRecordID),
			zap.String("fallback_id", fmt.Sprintf("%s:%s", record.Name, record.Type)))
			
		syncError = s.syncRecordToProviderSync(record, provider, "delete")
		if syncError != nil {
			zap.L().Error("从云厂商删除DNS记录失败", 
				zap.Uint("record_id", id),
				zap.String("provider", provider.Name),
				zap.String("cloud_record_id", record.CloudRecordID),
				zap.Error(syncError))
		} else {
			zap.L().Info("云厂商删除DNS记录成功", 
				zap.Uint("record_id", id),
				zap.String("provider", provider.Name),
				zap.String("cloud_record_id", record.CloudRecordID))
		}
	} else {
		zap.L().Warn("跳过云厂商删除，记录信息不完整", 
			zap.Uint("record_id", id),
			zap.String("cloud_record_id", record.CloudRecordID),
			zap.String("record_name", record.Name),
			zap.String("record_type", record.Type))
	...
	return s.db.Transaction(func(tx *gorm.DB) error {
		...
		if syncError != nil {
			...
			description += fmt.Sprintf("，云厂商删除失败: %v", syncError)
		}
		...
		// 如果云厂商删除失败，返回警告信息但不阻断本地删除
		if syncError != nil {
			zap.L().Warn("DNS记录已从本地删除，但云厂商删除失败，请手动检查",
				zap.Uint("record_id", id),
				zap.String("cloud_record_id", record.CloudRecordID),
				zap.Error(syncError))
		}
...
// SyncRecord 同步单个DNS记录到云厂商
func (s *RecordService) SyncRecord(recordID uint, actorID uint, clientIP string) error {
	// 获取记录信息
	...
	// 更新同步状态为处理中
	if err := s.recordRepo.UpdateSyncStatus(recordID, "syncing"); err != nil {
		zap.L().Error("更新DNS记录同步状态失败", zap.Error(err))
	}

	// 同步到云厂商（同步执行，确保状态正确更新）
	domain, _ := s.domainRepo.FindByID(record.DomainID)
	...
	// 执行同步操作（同步版本）
	syncErr := s.syncRecordToProviderSync(record, provider, "update")

	// 根据同步结果更新状态和记录日志
	var syncStatus, logStatus string
	if syncErr != nil {
		syncStatus = "error"
		logStatus = "failed"
		zap.L().Error("同步DNS记录到云厂商失败",
			zap.Uint("record_id", recordID),
			zap.Error(syncErr))

		// 更新失败状态
		if updateErr := s.recordRepo.UpdateSyncStatus(recordID, syncStatus); updateErr != nil {
			zap.L().Error("更新DNS记录同步状态失败", zap.Error(updateErr))
		}
	} else {
		syncStatus = "synced"
		logStatus = "success"
		zap.L().Info("同步DNS记录到云厂商成功",
			zap.Uint("record_id", recordID),
			zap.String("provider", provider.Name))
	...
	changeLog := &dns.ChangeLog{
		...
		ResourceID:   recordID,
		Action:       "sync",
		Description:  fmt.Sprintf("同步DNS记录: %s.%s (%s) - %s", record.Name, domainName, record.Type, logStatus),
		Status:       logStatus,
		ClientIP:     clientIP,
		TenantID:     record.TenantID,
		ActorID:      actorID,
	...
	if err := s.changeLogRepo.Create(changeLog); err != nil {
		zap.L().Error("记录DNS记录同步日志失败", zap.Error(err))
	}

	return syncErr
}

// BatchSyncRecords 批量同步DNS记录到云厂商
func (s *RecordService) BatchSyncRecords(recordIDs []uint, actorID uint, clientIP string) error {
	if len(recordIDs) == 0 {
		return errors.New("记录ID列表不能为空")
	}

	var successCount, failCount int

	for _, recordID := range recordIDs {
		if err := s.SyncRecord(recordID, actorID, clientIP); err != nil {
			zap.L().Error("同步DNS记录失败",
				zap.Uint("record_id", recordID),
				zap.Error(err))
			failCount++
		} else {
	...
	changeLog := &dns.ChangeLog{
		ResourceType: "record",
		ResourceID:   0, // 批量操作无单一资源ID
		Action:       "batch_sync",
		Description:  fmt.Sprintf("批量同步DNS记录: 成功%d个，失败%d个", successCount, failCount),
		Status:       "completed",
		ClientIP:     clientIP,
		TenantID:     0, // TODO: 从第一个记录获取
		ActorID:      actorID,
	}

	if err := s.changeLogRepo.Create(changeLog); err != nil {
		zap.L().Error("记录DNS批量同步日志失败", zap.Error(err))
	}

	if failCount > 0 {
		return fmt.Errorf("批量同步完成，其中%d个失败", failCount)
	}
...
func (s *RecordService) BatchDeleteRecords(recordIDs []uint, actorID uint, clientIP string) error {
	if len(recordIDs) == 0 {
		return errors.New("记录ID列表不能为空")
	}
	...
	for _, recordID := range recordIDs {
		if err := s.DeleteRecord(recordID, actorID, clientIP); err != nil {
			zap.L().Error("删除DNS记录失败",
				zap.Uint("record_id", recordID),
				zap.Error(err))
			failCount++
		} else {
	...
	changeLog := &dns.ChangeLog{
		...
		ResourceID:   0, // 批量操作无单一资源ID
		Action:       "batch_delete",
		Description:  fmt.Sprintf("批量删除DNS记录: 成功%d个，失败%d个", successCount, failCount),
		Status:       "completed",
		ClientIP:     clientIP,
		TenantID:     0, // TODO: 从第一个记录获取
		ActorID:      actorID,
	...
	if err := s.changeLogRepo.Create(changeLog); err != nil {
		zap.L().Error("记录DNS批量删除日志失败", zap.Error(err))
	}

	if failCount > 0 {
		return fmt.Errorf("批量删除完成，其中%d个失败", failCount)
	}
...
func (s *RecordService) syncRecordToProvider(record *dns.Record, provider *dns.Provider, action string) {
	...
	case "delete":
		...
		if targetID == "" {
			...
			zap.L().Warn("DNS记录缺少CloudRecordID，使用名称类型组合删除",
				zap.Uint("record_id", record.ID),
				zap.String("target_id", targetID))
		}
		err = driver.DeleteRecord(ctx, zoneName, targetID)
		if err != nil {
			// 如果使用CloudRecordID删除失败，尝试使用name:type组合
			if record.CloudRecordID != "" {
				fallbackID := fmt.Sprintf("%s:%s", record.Name, record.Type)
				zap.L().Warn("使用CloudRecordID删除失败，尝试使用名称类型组合",
					zap.String("cloud_record_id", record.CloudRecordID),
					zap.String("fallback_id", fallbackID),
					zap.Error(err))

				if fallbackErr := driver.DeleteRecord(ctx, zoneName, fallbackID); fallbackErr != nil {
					// 两种方式都失败，返回原始错误
					zap.L().Error("两种删除方式都失败",
						zap.String("cloud_record_id", record.CloudRecordID),
						zap.String("fallback_id", fallbackID),
						zap.Error(err),
					...
					// 备用方式成功，清除错误
					err = nil
					zap.L().Info("使用名称类型组合删除成功", zap.String("fallback_id", fallbackID))
				}
			}
		}
	}
...
// syncRecordToProviderSync 同步记录到DNS提供商（同步版本，返回错误）
func (s *RecordService) syncRecordToProviderSync(record *dns.Record, provider *dns.Provider, action string) error {
	// 创建上下文
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	creds, err := s.getProviderCredentials(provider)
	if err != nil {
		return fmt.Errorf("获取DNS提供商凭证失败: %w", err)
	}

	driver, err := dnsprovider.CreateDriver(provider.Type, provider.Name, creds)
	if err != nil {
		return fmt.Errorf("创建DNS驱动失败: %w", err)
	}
	...
	case "create":
		prio := optionalInt(record.Priority)
		_, err = driver.CreateRecord(ctx, zoneName, &dnsprovider.Record{
			Name:     record.Name,
			Type:     record.Type,
			Value:    record.Value,
			TTL:      record.TTL,
	...
	case "update":
		...
		// 如果没有云端ID，直接创建记录
		if record.CloudRecordID == "" {
			zap.L().Info("记录无云端ID，尝试创建", zap.Uint("record_id", record.ID))
			created, err2 := driver.CreateRecord(ctx, zoneName, &dnsprovider.Record{
				Name:     record.Name,
				Type:     record.Type,
				Value:    record.Value,
			...
			if err2 != nil {
				// 更新失败，可能记录在云端不存在，尝试创建
				zap.L().Warn("更新记录失败，尝试创建",
					zap.Uint("record_id", record.ID),
					zap.String("cloud_id", record.CloudRecordID),
					zap.Error(err2))

				created, createErr := driver.CreateRecord(ctx, zoneName, &dnsprovider.Record{
					Name:     record.Name,
					Type:     record.Type,
					Value:    record.Value,
					TTL:      record.TTL,
				...
				if createErr == nil && created != nil && created.ID != "" {
					...
					err = fmt.Errorf("更新失败: %w, 创建也失败: %w", err2, createErr)
				}
			} else {
				// 更新成功
				if updated != nil && updated.ID != "" && updated.ID != record.CloudRecordID {
					_ = s.recordRepo.UpdateCloudRecordID(record.ID, updated.ID)
				}
				err = nil
			}
			...
			if record.CloudRecordID != "" {
				...
				if fallbackErr := driver.DeleteRecord(ctx, zoneName, fallbackID); fallbackErr != nil {
					...
					zap.L().Error("两种删除方式都失败",
						...
						zap.Error(err),
						zap.NamedError("fallback_error", fallbackErr))
					return fmt.Errorf("删除DNS记录失败 (CloudRecordID: %s): %w", record.CloudRecordID, err)
				} else {
					// 备用方式成功，清除错误
					err = nil
					zap.L().Info("使用名称类型组合删除成功", zap.String("fallback_id", fallbackID))
...
func (s *RecordService) getProviderCredentials(provider *dns.Provider) (map[string]string, error) {
	// 直接使用解密逻辑
	return s.decryptCredentials(provider)
}

// decryptCredentials 解密凭证信息（从ProviderService复制的逻辑）
func (s *RecordService) decryptCredentials(provider *dns.Provider) (map[string]string, error) {
...
func (s *RecordService) decryptPassword(encryptedText string) (string, error) {
	if encryptedText == "" {
		return "", nil
	}
...
func (s *RecordService) SyncDomainRecords(params *SyncDomainRecordsParams) (*RecordSyncResult, error) {
	...
	// 执行实际同步操作
	if err := s.executeSyncOperations(domain, provider, toAdd, toUpdate, toDelete, remoteRecords, result, params); err != nil {
...
func (s *RecordService) recordNeedsUpdate(local *dns.Record, remote *RemoteRecord) bool {
	...
	// 重要：检查CloudRecordID是否需要更新
	if local.CloudRecordID != remote.RecordID && remote.RecordID != "" {
		return true
	}
	return false
}
...
func (s *RecordService) executeSyncOperations(
	...
	for _, remoteRecord := range toAdd {
		newRecord := &dns.Record{
			...
			Status:        "active",
			SyncStatus:    "synced",
			CloudRecordID: remoteRecord.RecordID, // 重要：设置云端记录ID
	...
	for _, localRecord := range toUpdate {
		// 找到对应的远程记录，更新CloudRecordID
		key := fmt.Sprintf("%s:%s", localRecord.Name, localRecord.Type)
		for _, remoteRecord := range toAdd {
			if fmt.Sprintf("%s:%s", remoteRecord.Name, remoteRecord.Type) == key {
				// 不应该出现在这里，跳过
				continue
			}
		}
		...
		// 查找匹配的远程记录
		var matchedRemoteRecord *RemoteRecord
		for _, remoteRecord := range remoteRecords {
			if remoteRecord.Name == localRecord.Name && remoteRecord.Type == localRecord.Type {
				matchedRemoteRecord = remoteRecord
				break
			}
		}

		// 如果找到匹配的远程记录，更新相关字段
		if matchedRemoteRecord != nil {
			// 更新记录内容和CloudRecordID
			localRecord.Value = matchedRemoteRecord.Value
			localRecord.TTL = matchedRemoteRecord.TTL
			if matchedRemoteRecord.Priority > 0 {
				localRecord.Priority = &matchedRemoteRecord.Priority
			}
			if matchedRemoteRecord.Weight > 0 {
				localRecord.Weight = &matchedRemoteRecord.Weight
			}
			if matchedRemoteRecord.Port > 0 {
				localRecord.Port = &matchedRemoteRecord.Port
			}
			// 重要：更新CloudRecordID
			if matchedRemoteRecord.RecordID != "" {
				localRecord.CloudRecordID = matchedRemoteRecord.RecordID
			}
		}
		...
		zap.L().Info("更新DNS记录成功",
			...
			zap.String("cloud_record_id", localRecord.CloudRecordID))
	}
...
// GetDebugSyncStatus 获取调试同步状态信息
func (s *RecordService) GetDebugSyncStatus(tenantID uint) (map[string]interface{}, error) {
	// 获取该租户的所有记录
	records, err := s.recordRepo.FindByTenantID(tenantID)
	if err != nil {
	...
	// 统计同步状态
	syncStatusCounts := make(map[string]int)
	recordDetails := make([]map[string]interface{}, 0)

	for _, record := range records {
		syncStatusCounts[record.SyncStatus]++

		recordDetail := map[string]interface{}{
			"id":              record.ID,
			"name":            record.Name,
			"type":            record.Type,
			"value":           record.Value,
			"sync_status":     record.SyncStatus,
			"cloud_record_id": record.CloudRecordID,
			"created_at":      record.CreatedAt,
			"updated_at":      record.UpdatedAt,
		}

		if record.LastSyncAt != nil {
			recordDetail["last_sync_at"] = record.LastSyncAt.Time
		}

		if record.Domain != nil {
			recordDetail["domain_name"] = record.Domain.Name
			recordDetail["domain_id"] = record.Domain.ID
		}

		if record.Provider != nil {
			recordDetail["provider_name"] = record.Provider.Name
			recordDetail["provider_type"] = record.Provider.Type
			recordDetail["provider_id"] = record.Provider.ID
		}
	...
	// 获取域名信息
	domains, err := s.domainRepo.FindByTenantID(tenantID)
	if err != nil {
		return nil, fmt.Errorf("获取域名失败: %w", err)
	}

	domainDetails := make([]map[string]interface{}, 0)
	for _, domain := range domains {
		domainDetail := map[string]interface{}{
			"id":           domain.ID,
			"name":         domain.Name,
			"provider_id":  domain.ProviderID,
			"registrar_id": domain.RegistrarID,
		}

		if domain.Provider != nil {
			domainDetail["provider_name"] = domain.Provider.Name
			domainDetail["provider_type"] = domain.Provider.Type
		}
...
// FixSyncStatus 修复DNS记录的同步状态
func (s *RecordService) FixSyncStatus(tenantID uint) (map[string]interface{}, error) {
	// 获取该租户的所有记录
	records, err := s.recordRepo.FindByTenantID(tenantID)
	if err != nil {
		return nil, fmt.Errorf("获取记录失败: %w", err)
	...
	for _, record := range records {
		// 修复空的、NULL的或错误的同步状态
		if record.SyncStatus == "" || record.SyncStatus == "pending" || record.SyncStatus == "syncing" ||
			record.SyncStatus == "failed" || record.SyncStatus == "error" {
			if err := s.recordRepo.UpdateSyncStatus(record.ID, "synced"); err != nil {
				errors = append(errors, fmt.Sprintf("修复记录%d失败: %v", record.ID, err))
			} else {
				fixedCount++
			}
		}

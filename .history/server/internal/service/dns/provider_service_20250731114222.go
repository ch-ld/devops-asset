...
func (s *ProviderService) TestProvider(id uint) (*dns.TestResult, error) {
	provider, err := s.providerRepo.FindByID(id)
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, errors.New("DNS提供商不存在")
	...
	// 测试连接
	result := dns.TestDriver(provider.Type, provider.Name, credentials)
...
func (s *ProviderService) providerToJSON(provider *dns.Provider) []byte {
	...
	data, err := encryption.ToJSON(&providerCopy)
	if err != nil {

...
func (s *SyncService) createDriver(provider *dns.Provider) (SimpleDNSDriver, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to create provider driver: %w", err)
	}

	return &simpleDriverAdapter{inner: driver}, nil
}

// simpleDriverAdapter 将通用 Driver 适配为简化接口 SimpleDNSDriver
type simpleDriverAdapter struct {
	inner dnsprovider.Driver
}

func (a *simpleDriverAdapter) GetName() string {
	if a.inner.GetInfo() != nil {
		return a.inner.GetInfo().Name
	}
	return ""
}

func (a *simpleDriverAdapter) TestConnection(ctx context.Context) *TestResult {
	tr := a.inner.Test(ctx)
	return &TestResult{Success: tr.Success, Message: tr.ErrorMsg, Latency: tr.Latency}
}

func (a *simpleDriverAdapter) ListRecords(ctx context.Context, domainName string) ([]*Record, error) {
	recs, err := a.inner.ListRecords(ctx, domainName, nil)
	if err != nil {
		// 对未实现的驱动返回空列表，避免阻塞主流程
		if err == dnsprovider.ErrNotImplemented {
			return []*Record{}, nil
		}
		return nil, err
	}
	...
	for i := range recs {
		...
		out = append(out, &Record{
			...
			Value:    r.Value,
			TTL:      r.TTL,
			Priority: func() int { if r.Priority != nil { return *r.Priority }; return 0 }(),
			Status:   "",
		})
	}
	return out, nil
...
func (a *simpleDriverAdapter) CreateRecord(ctx context.Context, domainName string, record *Record) (*Record, error) {
	prio := record.Priority
	provRec := &dnsprovider.Record{
		Name:     record.Name,
		Type:     record.Type,
		Value:    record.Value,
		TTL:      record.TTL,
	...
	res, err := a.inner.CreateRecord(ctx, domainName, provRec)
	if err != nil {
		return nil, err
	}
	return &Record{
		...
		Value:    res.Value,
		TTL:      res.TTL,
		Priority: func() int { if res.Priority != nil { return *res.Priority }; return 0 }(),
		Status:   "",
	}, nil
}

func (a *simpleDriverAdapter) UpdateRecord(ctx context.Context, domainName string, record *Record) (*Record, error) {
	prio := record.Priority
	provRec := &dnsprovider.Record{
		ID:       record.ID,
		Name:     record.Name,
	...
	res, err := a.inner.UpdateRecord(ctx, domainName, provRec)
	if err != nil {
		return nil, err
	}
	return &Record{
...
func (a *simpleDriverAdapter) DeleteRecord(ctx context.Context, domainName string, recordID string) error {
	return a.inner.DeleteRecord(ctx, domainName, recordID)
}

// decryptCredentials 解密 Provider 凭证
func (s *SyncService) decryptCredentials(provider *dns.Provider) (map[string]string, error) {
	...
	if len(config.GlobalConfig.App.AesKeys) > 0 {
		keys := make([][]byte, 0, len(config.GlobalConfig.App.AesKeys))
		for _, k := range config.GlobalConfig.App.AesKeys {
			keys = append(keys, []byte(k))
		}
		decrypted, err = encryption.DecryptAESWithKeys(provider.CredentialsEnc, keys)
	} else if config.GlobalConfig.App.AesKey != "" {
		decrypted, err = encryption.DecryptAES(provider.CredentialsEnc, []byte(config.GlobalConfig.App.AesKey))
	} else {
	...
	if err := json.Unmarshal([]byte(decrypted), &creds); err != nil {
		return nil, fmt.Errorf("parse credentials json failed: %w", err)
	}
	return creds, nil
}

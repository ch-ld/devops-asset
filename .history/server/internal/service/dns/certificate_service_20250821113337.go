...
type CertificateRequest struct {
	...
	KeyType           string   `json:"key_type"`           // RSA1024, RSA2048, RSA3072, RSA4096, EC256, EC384
	ValidDays         int      `json:"valid_days"`         // 证书有效天数
	AutoRenew         bool     `json:"auto_renew"`         // 是否自动续期
	RenewDays         int      `json:"renew_days"`         // 提前续期天数
	ChallengeType     string   `json:"challenge_type"`     // 验证方式: dns, cname, http
	ScheduleTime      string   `json:"schedule_time"`      // 定时触发时间
	NotificationType  string   `json:"notification_type"`  // 通知类型: default, custom
	NotificationEmail string   `json:"notification_email"` // 自定义通知邮箱
	DeployHosts       []uint   `json:"deploy_hosts"`       // 部署主机ID列表
...
func (s *CertificateService) IssueCertificate(ctx context.Context, req *CertificateRequest, userID uint, clientIP string) (*CertificateInfo, error) {
	...
	var err error
	tenantID := uint(1) // TODO: 从上下文获取租户ID
	...
	if req.DomainID > 0 {
		...
		// 对于通配符域名或新域名，尝试查找或创建域名记录
		domainName := req.Domains[0]
		isWildcard, parentDomain, level := dns.ParseWildcardDomain(domainName)

		// 查找是否已存在该域名记录
		existingDomain, err := s.domainRepo.FindByName(domainName)
		if err == nil {
			...
			// 如果是通配符域名，需要找到父域名的Provider配置
			if isWildcard {
				parentDomainRecord, err := s.domainRepo.FindByName(parentDomain)
				if err == nil && parentDomainRecord.ProviderID != nil {
					domain.ProviderID = parentDomainRecord.ProviderID
				} else {
					// 使用请求中的ProviderID
					domain.ProviderID = &req.ProviderID
				}
			} else {
				domain.ProviderID = &req.ProviderID
			}
	...
	// 申请证书
	s.logger.Info("Requesting certificate for domains", zap.Strings("domains", req.Domains))
	...
	if err != nil {
		s.logger.Error("Failed to obtain certificate",
			zap.Strings("domains", req.Domains),
			zap.Error(err))
		return nil, fmt.Errorf("failed to obtain certificate: %w", err)
	}

	s.logger.Info("Certificate obtained successfully",
		zap.Strings("domains", req.Domains),
		zap.String("certificate_url", certificates.CertURL))

	// 保存证书到数据库
	now := time.Now()
	validDays := req.ValidDays
	if validDays <= 0 {
		validDays = 90 // Let's Encrypt证书有效期90天
	}
	...
	// 处理域名名称，通配符证书可能没有关联域名
	var domainName string
	if domain != nil {
		domainName = domain.Name
	} else {
		// 对于通配符证书，从证书的CommonName提取域名
...
// deployToSingleHost 部署证书到单个主机
func (s *CertificateService) deployToSingleHost(deployment *dns.CertificateDeployment, certData, keyData, chainData []byte) {
	s.logger.Info("Deploying certificate to single host",
		zap.Uint("deployment_id", deployment.ID),
		zap.Uint("host_id", deployment.HostID))

	// 更新部署状态为处理中
	deployment.Status = "processing"
	s.certificateRepo.UpdateDeployment(deployment)

	// TODO: 这里需要与主机模块集成，实现实际的文件传输和部署
	// 目前先实现一个模拟的部署过程
	...
	// 构造证书文件内容
	certFileName := fmt.Sprintf("%s.crt", deployment.Certificate.CommonName)
	keyFileName := fmt.Sprintf("%s.key", deployment.Certificate.CommonName)
	chainFileName := fmt.Sprintf("%s-chain.crt", deployment.Certificate.CommonName)

	// 模拟文件上传和服务重启
	s.logger.Info("Simulating certificate file deployment",
		zap.String("cert_file", certFileName),
		zap.String("key_file", keyFileName),
		zap.String("chain_file", chainFileName),
		zap.String("deploy_path", deployment.DeployPath))
	...
	err := s.certificateRepo.UpdateDeployment(deployment)
	if err != nil {
	...
	s.logger.Info("Certificate deployed successfully to host",
		zap.Uint("deployment_id", deployment.ID),
		zap.Uint("host_id", deployment.HostID))
}
...
func (s *CertificateService) RenewCertificate(ctx context.Context, id uint, userID uint, clientIP string) (*CertificateInfo, error) {
	...
	// 获取域名信息（如果有关联域名）
	var domain *dns.Domain
	var providerID uint = 1                // 默认使用第一个DNS Provider
	var email string = "admin@example.com" // 默认邮箱
...
func (s *CertificateService) DownloadCertificate(ctx context.Context, id uint, format string) (*CertificateDownloadInfo, error) {
	...
	case "nginx":
		// Nginx格式：证书 + 证书链合并为一个文件
		content = append(content, certData...)
		if len(chainData) > 0 {
			content = append(content, []byte("\n")...)
			content = append(content, chainData...)
		}
		filename = fmt.Sprintf("%s_%d_nginx.crt", cert.CommonName, cert.ID)
		mimeType = "application/x-x509-ca-cert"

	case "apache":
		// Apache格式：分别提供证书、私钥和证书链文件
		// 这里返回证书文件，私钥和链文件需要单独下载
		content = certData
		filename = fmt.Sprintf("%s_%d_apache.crt", cert.CommonName, cert.ID)
		mimeType = "application/x-x509-ca-cert"

	case "iis":
		// IIS格式：PKCS#12格式（需要密码）
		// 这里暂时返回PEM格式，实际应该转换为PKCS#12
		content = append(content, certData...)
		content = append(content, []byte("\n")...)
		content = append(content, keyData...)
		if len(chainData) > 0 {
		...
		filename = fmt.Sprintf("%s_%d_iis.pem", cert.CommonName, cert.ID)
		mimeType = "application/x-pem-file"

	case "tomcat":
		// Tomcat格式：Java KeyStore格式
		// 这里暂时返回PEM格式，实际应该转换为JKS
		content = append(content, certData...)
		content = append(content, []byte("\n")...)
		content = append(content, keyData...)
		if len(chainData) > 0 {
		...
		filename = fmt.Sprintf("%s_%d_tomcat.pem", cert.CommonName, cert.ID)
		mimeType = "application/x-pem-file"
...
func (s *CertificateService) DeleteCertificate(ctx context.Context, id, tenantID, userID uint) error {
	s.logger.Info("Deleting certificate", zap.Uint("id", id), zap.Uint("tenant_id", tenantID))
	...
	if cert.TenantID != tenantID {
		return fmt.Errorf("无权限删除该证书")
	}

	// 删除证书
	if err := s.certificateRepo.Delete(id); err != nil {
		s.logger.Error("Failed to delete certificate",
			zap.Uint("certificate_id", id),
			zap.Error(err))
		return fmt.Errorf("删除证书失败: %w", err)
	}

	s.logger.Info("Certificate deleted successfully", zap.Uint("id", id))
	return nil
}

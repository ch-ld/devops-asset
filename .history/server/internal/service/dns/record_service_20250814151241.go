...
func (s *RecordService) syncRecordToProviderSync(record *dns.Record, provider *dns.Provider, action string) error {
	...
	// 解密凭证并创建驱动
	credentials, err := s.decryptCredentials(provider.Credentials)
	if err != nil {
		return fmt.Errorf("解密DNS提供商凭证失败: %w", err)
	}

	factory := dnsprovider.GetFactory()
	driver, err := factory.CreateDriver(provider.Type, credentials)
	if err != nil {

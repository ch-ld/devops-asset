...
func (s *ProviderService) encryptCredentials(provider *dns.Provider) error {
	...
	if len(config.GlobalConfig.App.AesKeys) > 0 {
		...
		encrypted, err := encryption.EncryptAESWithKeys([]byte(provider.CredentialsEnc), keys)
		if err != nil {
			return fmt.Errorf("加密凭证信息失败: %w", err)
		}
		...
		// 使用单密钥加密
		encrypted, err := encryption.EncryptAES([]byte(provider.CredentialsEnc), []byte(config.GlobalConfig.App.AesKey))
		if err != nil {
			return fmt.Errorf("加密凭证信息失败: %w", err)
		}
		provider.CredentialsEnc = encrypted
	} else {
		return fmt.Errorf("未配置加密密钥")
	}

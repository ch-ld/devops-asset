...
func (p *DNSProvider) extractZoneName(domain string) string {
	...
	// 对于通配符域名：*.example.com -> example.com
	// 对于多级通配符：*.test.devopsgo.shop -> devopsgo.shop
	...
	if len(parts) >= 2 {
		// 对于多级域名，需要智能识别根域名
		// 常见的顶级域名：com, net, org, cn, com.cn 等
		if len(parts) >= 3 && (parts[len(parts)-1] == "cn" &&
			(parts[len(parts)-2] == "com" || parts[len(parts)-2] == "net" || parts[len(parts)-2] == "org")) {
			// 处理 .com.cn, .net.cn, .org.cn 等二级顶级域名

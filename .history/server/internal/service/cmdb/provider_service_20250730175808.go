...
func (s *ProviderService) SyncResourcesMultiRegion(provider *model.Provider, regions []string, groupID *uint) (*SyncResult, error) {
	...
	for _, region := range regions {
		...
		// 创建该区域的临时Provider对象，使用解密后的密钥
		regionProvider := &model.Provider{
			Name:      provider.Name,
			Type:      provider.Type,
			AccessKey: provider.AccessKey,
			SecretKey: decryptedSecret, // 使用解密后的密钥
			Region:    region,          // 使用指定的区域
		}
		regionProvider.ID = provider.ID
		...
		for _, host := range hosts {
			...
			if existingHost != nil {
				...
				existingHost.Configuration = host.Configuration
				existingHost.OS = host.OS
				existingHost.Region = region
				existingHost.ProviderType = host.ProviderType
				existingHost.ResourceType = host.ResourceType
				existingHost.ExpiredAt = host.ExpiredAt
				providerID := provider.ID
				existingHost.ProviderID = &providerID
				if groupID != nil {

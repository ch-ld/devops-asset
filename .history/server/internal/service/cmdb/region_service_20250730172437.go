...
func (s *RegionService) GetRegionsByProvider(providerType string) ([]string, error) {
	...
	// 如果数据库中没有区域数据，初始化默认区域到数据库
	if len(regions) == 0 {
		err = s.initDefaultRegions(providerType)
		if err != nil {
			zap.L().Error("初始化默认区域失败", zap.String("provider", providerType), zap.Error(err))
			// 如果初始化失败，返回内存中的默认区域作为备选
		...
		err = s.db.Where("provider_type = ? AND status = ?", providerType, "active").
			...
			Find(&regions).Error
		if err != nil {
			return nil, fmt.Errorf("查询初始化后的区域失败: %w", err)
		}
	}

...
func (s *HostGroupService) buildTree(groups []cmdb.HostGroup) []cmdb.HostGroup {
	// 创建一个map来快速查找节点，使用副本避免修改原始数据
	groupMap := make(map[uint]*cmdb.HostGroup)
	groupCopies := make([]cmdb.HostGroup, len(groups))

	// 创建副本并建立映射
	for i, group := range groups {
		groupCopies[i] = group
		groupCopies[i].Children = []cmdb.HostGroup{} // 初始化children
		groupMap[group.ID] = &groupCopies[i]
	}

	// 构建树状结构
	var tree []cmdb.HostGroup
	for _, group := range groupCopies {
		if group.ParentID == nil {
			// 根节点

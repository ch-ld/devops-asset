...
func (s *HostGroupService) calculateTotalHostCount(groups *[]cmdb.HostGroup) {
	for i := range *groups {
		...
		if len(group.Children) > 0 {
			...
			// 计算子分组的主机总数
			var childrenHostCount int64
			for _, child := range group.Children {
				childrenHostCount += child.HostCount
			}

			// 当前分组的主机数量 = 直接归属的主机数量 + 子分组的主机数量
			// 注意：这里保持HostCount为直接归属的主机数量，添加一个新字段TotalHostCount
			// 但由于模型中没有TotalHostCount字段，我们将在前端处理这个逻辑
		}
	}
}

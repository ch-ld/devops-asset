...
func (s *RegionService) SyncRegionsFromProvider(provider *cmdb_model.Provider) error {
...
func (s *RegionService) SyncAllProviderRegions() error {
	// 获取所有云账号
	var providers []cmdb_model.Provider
	if err := s.db.Find(&providers).Error; err != nil {
		return fmt.Errorf("获取云账号列表失败: %w", err)
	}

	// 按云厂商类型分组，避免重复同步
	providerTypes := make(map[string]*cmdb_model.Provider)

...
func (s *HostService) SyncAllProviderHosts() error {
	...
	for _, provider := range providers {
		if err := s.SyncHostsFromCloud(provider.ID); err != nil {
			// 记录错误但继续执行其他云账号的同步
			zap.L().Error("Failed to sync hosts from cloud provider",
...
// ValidateProvider 验证云账号是否存在
func (s *HostService) ValidateProvider(providerID uint) error {
	_, err := s.providerRepo.FindByID(providerID)
	if err != nil {

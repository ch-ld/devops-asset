...
func (r *ProviderRepository) FindDefault(tenantID uint) (*dns.Provider, error) {
	var provider dns.Provider
	err := r.db.Where("tenant_id = ? AND is_default = ? AND status = ?", tenantID, true, "active").First(&provider).Error
	if err != nil {
		return nil, err
	}
	return &provider, nil

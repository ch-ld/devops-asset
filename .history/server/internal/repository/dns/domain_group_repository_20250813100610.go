...
// CountByTenant 根据租户ID统计分组数量
func (r *DomainGroupRepository) CountByTenant(tenantID uint) (int64, error) {
	var count int64
	err := r.db.Model(&dns.DomainGroup{}).Where("tenant_id = ?", tenantID).Count(&count).Error
	return count, err
}

// CountByStatus 根据租户ID和状态统计分组数量
func (r *DomainGroupRepository) CountByStatus(tenantID uint, status string) (int64, error) {
	var count int64
	err := r.db.Model(&dns.DomainGroup{}).Where("tenant_id = ? AND status = ?", tenantID, status).Count(&count).Error
	return count, err
}

...
func (r *DomainRepository) FindByID(id uint) (*dns.Domain, error) {
	...
	err := r.db.Preload("Group").Preload("Tags").Preload("Provider").First(&domain, id).Error
	if err != nil {
		return nil, err
	}
	return &domain, nil
...
func (r *DomainRepository) FindByName(name string) (*dns.Domain, error) {
	var domain dns.Domain
	err := r.db.Preload("Group").Preload("Tags").Preload("Provider").Where("name = ?", name).First(&domain).Error
	if err != nil {
		return nil, err
	}
	return &domain, nil
...
func (r *DomainRepository) FindAll() ([]*dns.Domain, error) {
	...
	err := r.db.Preload("Group").Preload("Tags").Preload("Provider").Find(&domains).Error
	return domains, err
}
...
func (r *DomainRepository) FindByTenantID(tenantID uint) ([]*dns.Domain, error) {
	var domains []*dns.Domain
	err := r.db.Preload("Group").Preload("Tags").Preload("Provider").Where("tenant_id = ?", tenantID).Find(&domains).Error
	return domains, err
}
...
func (r *DomainRepository) FindByGroupID(groupID uint) ([]*dns.Domain, error) {
	var domains []*dns.Domain
	err := r.db.Preload("Group").Preload("Tags").Preload("Provider").Where("group_id = ?", groupID).Find(&domains).Error
	return domains, err
}
...
func (r *DomainRepository) FindByStatus(status string) ([]*dns.Domain, error) {
	var domains []*dns.Domain
	err := r.db.Preload("Group").Preload("Tags").Preload("Provider").Where("status = ?", status).Find(&domains).Error
	return domains, err
}
...
func (r *DomainRepository) FindExpiring(days int) ([]*dns.Domain, error) {
	var domains []*dns.Domain
	err := r.db.Preload("Group").Preload("Tags").Preload("Provider").
...
func (r *DomainRepository) Search(keyword string, limit, offset int) ([]*dns.Domain, int64, error) {
	...
	query := r.db.Preload("Group").Preload("Tags").Preload("Provider").Model(&dns.Domain{})
...
// CountByStatusAndTenant 统计各状态域名数量（按租户过滤）
func (r *DomainRepository) CountByStatusAndTenant(tenantID uint) (map[string]int64, error) {
	var results []struct {
		Status string
		Count  int64
	}
...
func (r *DomainRepository) UpsertByName(domain *dns.Domain) error {
	return r.db.Clauses(clause.OnConflict{
		...
		DoUpdates: clause.AssignmentColumns([]string{"registrar_id", "provider_id", "registrar_type", "updated_by", "updated_at"}),
	}).Create(domain).Error
}
...
func (r *DomainRepository) FindByProviderID(providerID uint) ([]*dns.Domain, error) {
	var domains []*dns.Domain
	err := r.db.Preload("Group").Preload("Tags").Preload("Provider").Where("registrar_id = ?", providerID).Find(&domains).Error
	return domains, err
}
...
// GetDomainCountByGroups 获取每个分组的域名数量统计
func (r *DomainRepository) GetDomainCountByGroups(tenantID uint) (map[uint]int64, error) {
	type Result struct {

...
// UpdateStatus 更新证书状态
func (r *CertificateRepository) UpdateStatus(id uint, status string) error {
...
// CreateDeployment 创建证书部署记录
func (r *CertificateRepository) CreateDeployment(deployment *dns.CertificateDeployment) error {
	return r.DB.Create(deployment).Error
}

// UpdateDeployment 更新证书部署记录
func (r *CertificateRepository) UpdateDeployment(deployment *dns.CertificateDeployment) error {
	return r.DB.Model(deployment).Updates(deployment).Error
}

// FindDeploymentsByID 根据证书ID查找部署记录
func (r *CertificateRepository) FindDeploymentsByID(certificateID uint) ([]*dns.CertificateDeployment, error) {
	var deployments []*dns.CertificateDeployment
	err := r.DB.Where("certificate_id = ?", certificateID).Find(&deployments).Error

...
// FindByCommonName 根据主域名查找证�?func (r *CertificateRepository) FindByCommonName(commonName string) ([]*dns.Certificate, error) {
	var certs []*dns.Certificate
	err := r.DB.Preload("Domain").Where("common_name = ?", commonName).Find(&certs).Error
	return certs, err
}

// FindByStatus 根据状态查找证�?func (r *CertificateRepository) FindByStatus(status string) ([]*dns.Certificate, error) {
	var certs []*dns.Certificate
...
// FindExpiring 查找即将过期的证�?func (r *CertificateRepository) FindExpiring(days int) ([]*dns.Certificate, error) {
	var certs []*dns.Certificate
...
// CountByStatus 统计各状态证书数�?func (r *CertificateRepository) CountByStatus() (map[string]int64, error) {
	var results []struct {
		Status string
		Count  int64
	}
...
// ExistsByCommonName 检查证书是否存�?func (r *CertificateRepository) ExistsByCommonName(commonName string) (bool, error) {
	var count int64

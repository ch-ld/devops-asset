...
// FindByInstanceIDAndRegion 根据实例ID和区域查询主机
func (r *HostRepository) FindByInstanceIDAndRegion(instanceID, region string) (*cmdb.Host, error) {
	var host cmdb.Host
	if err := r.db.Where("instance_id = ? AND region = ?", instanceID, region).First(&host).Error; err != nil {
		return nil, err
	}
	return &host, nil
}

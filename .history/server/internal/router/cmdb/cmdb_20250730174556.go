...
func InitCmdbRoutes(r *gin.RouterGroup, db *gorm.DB) {
	...
	// Handlers
	regionSvc := svc.NewRegionService(db)
	providerHandler := cmdb.NewProviderHandler(providerSvc, regionSvc)
	...
	cmdbGroup.Use(middleware.JWTAuth())
	{
		...
		providerGroup := cmdbGroup.Group("/providers")
		{
			...
			providerGroup.POST("/regions-with-credentials", providerHandler.GetProviderRegionsWithCredentials)
			...
			providerGroup.GET("/:id/regions", providerHandler.GetRegions)
			providerGroup.POST("/:id/sync-regions", providerHandler.SyncRegions)
			providerGroup.POST("/:id/sync", providerHandler.SyncResources)
			providerGroup.POST("/:id/validate", providerHandler.ValidateProviderCredentials)
		}
	...
	{
		wsGroup.GET("/ssh", webSshHandler.HandleWebSSH)
	}

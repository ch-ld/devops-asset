...
func InitDnsRoutes(r *gin.RouterGroup, db *gorm.DB) {
	...
	// 创建DNS定时任务服务（需要证书相关的repository和service，暂时传nil）
	CronSvc = svc.NewCronService(
		db,
		domainRepo,
		nil, // certificateRepo - TODO: 等证书功能实现后替换
		recordRepo,
		providerRepo,
		DomainSvc,
	...
	// 创建证书服务和Handler
	certificateService := dnsService.NewCertificateService(
	...
	// 创建HTTPS监控服务和Handler
	httpsMonitorService := dnsService.NewHTTPSMonitorService(

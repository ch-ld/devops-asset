...
func InitDnsRoutes(r *gin.RouterGroup, db *gorm.DB) {
	...
	DomainGroupSvc = svc.NewDomainGroupService(domainGroupRepo, domainRepo, changeLogRepo, db)
	...
	recordStatHandler := dns.NewRecordStatHandler(RecordSvc)
	recordImpExpHandler := dns.NewRecordImportExportHandler(RecordSvc)
	...
	{
		// 域名分组管理路由
		fmt.Println("[DNS路由] 注册域名分组管理路由...")
		domainGroupGroup := dnsGroup.Group("/domain-groups")
		domainGroupGroup.Use(middleware.JWTAuth())
		{
			domainGroupGroup.POST("", domainGroupHandler.CreateDomainGroup)                  // 创建域名分组
			domainGroupGroup.GET("", domainGroupHandler.ListDomainGroups)                    // 获取域名分组列表
			domainGroupGroup.GET("/tree", domainGroupHandler.GetDomainGroupTree)             // 获取域名分组树
			domainGroupGroup.GET("/:id", domainGroupHandler.GetDomainGroup)                  // 获取域名分组详情
			domainGroupGroup.PUT("/:id", domainGroupHandler.UpdateDomainGroup)               // 更新域名分组
			domainGroupGroup.DELETE("/:id", domainGroupHandler.DeleteDomainGroup)            // 删除域名分组
			domainGroupGroup.PUT("/:id/move", domainGroupHandler.MoveDomainGroup)            // 移动域名分组
			domainGroupGroup.GET("/statistics", domainGroupHandler.GetDomainGroupStatistics) // 获取分组统计
		}
		...
		domainGroup.Use(middleware.JWTAuth())
		{
			...
			// WHOIS查询功能
			domainGroup.GET("/:id/whois", domainHandler.GetDomainWhois)           // 获取域名WHOIS信息
			domainGroup.GET("/:id/whois/report", domainHandler.ExportWhoisReport) // 导出WHOIS报告

			// 导入导出功能
			domainGroup.GET("/template/csv", domainHandler.DownloadCSVTemplate)     // 下载CSV模板
			domainGroup.GET("/template/excel", domainHandler.DownloadExcelTemplate) // 下载Excel模板
			...
			// 批量操作
			domainGroup.POST("/batch", domainHandler.BatchDeleteDomains) // 批量删除域名
		}
		...
		recordGroup.Use(middleware.JWTAuth())
		{
			...
			recordGroup.POST("/:id/sync", recordHandler.SyncRecord)             // 同步DNS记录
			recordGroup.POST("/batch/sync", recordHandler.BatchSyncRecords)     // 批量同步DNS记录
			recordGroup.POST("/batch/delete", recordHandler.BatchDeleteRecords) // 批量删除DNS记录

			// 调试接口（仅开发环境）
			recordGroup.GET("/debug/sync-status", recordHandler.DebugSyncStatus)    // 调试同步状态
			recordGroup.POST("/debug/fix-sync-status", recordHandler.FixSyncStatus) // 修复同步状态
			// 统计接口：按域名ID批量统计记录数
			recordGroup.POST("/count-by-domain", recordStatHandler.CountByDomainIDs)
		}
		...
		{
			importExportGroup.POST("/records/import", recordImpExpHandler.ImportRecordsFromJSON)
			importExportGroup.POST("/records/export", recordImpExpHandler.ExportRecords)
		}

		// 模板下载（不需要认证）
		dnsGroup.GET("/records/template/csv", recordImpExpHandler.DownloadRecordCSVTemplate)
		dnsGroup.GET("/records/template/excel", recordImpExpHandler.DownloadRecordExcelTemplate)
		...
		certGroup.Use(middleware.JWTAuth())
		{
			...
			certGroup.DELETE("/:id", certificateHandler.DeleteCertificate)         // 删除证书
			...
			// 证书上传和验证功能
			certGroup.POST("/upload", certificateHandler.UploadCertificate)                 // 上传证书
			certGroup.POST("/validate-certificate", certificateHandler.ValidateCertificate) // 验证证书
			...
			// 批量操作功能
			certGroup.POST("/batch-renew", certificateHandler.BatchRenewCertificates)   // 批量续期证书
			certGroup.POST("/batch-delete", certificateHandler.BatchDeleteCertificates) // 批量删除证书
			certGroup.GET("/batch-export", certificateHandler.BatchExportCertificates)  // 批量导出证书
			certGroup.POST("/batch-deploy", certificateHandler.BatchDeployCertificates) // 批量部署证书
		}
		...
		// 通知管理路由
		fmt.Println("[DNS路由] 注册通知管理路由...")
		notificationGroup := dnsGroup.Group("/notifications")
		notificationGroup.Use(middleware.JWTAuth())
		{
			// TODO: 实现通知Handler后启用这些路由
			// notificationGroup.POST("", notificationHandler.CreateNotification)           // 创建通知
			// notificationGroup.GET("", notificationHandler.ListNotifications)             // 获取通知列表
			// notificationGroup.GET("/:id", notificationHandler.GetNotification)           // 获取通知详情
			// notificationGroup.PUT("/:id", notificationHandler.UpdateNotification)        // 更新通知
			// notificationGroup.DELETE("/:id", notificationHandler.DeleteNotification)     // 删除通知
			// notificationGroup.POST("/:id/read", notificationHandler.MarkAsRead)          // 标记为已读
			// notificationGroup.POST("/batch-read", notificationHandler.BatchMarkAsRead)   // 批量标记为已读
			// notificationGroup.DELETE("/batch", notificationHandler.BatchDelete)         // 批量删除
			// notificationGroup.GET("/unread-count", notificationHandler.GetUnreadCount)  // 获取未读数量
		}
	}

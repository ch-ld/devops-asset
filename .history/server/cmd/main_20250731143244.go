...
func main() {
	...
	stdlog.Println("[启动日志] 步骤10.3: 注册DNS路由")
	dns.InitDnsRoutes(apiGroup, mysql.GetClient()) // 注册DNS路由
	...
	// 11.4 启动DNS定时任务服务
	stdlog.Println("[启动日志] 步骤11.4: 启动DNS定时任务服务")
	go func() {
		stdlog.Println("[启动日志] 开始启动DNS定时任务服务...")
		zap.L().Info("Starting DNS cron service...")
		if err := dns.CronSvc.Start(); err != nil {
			stdlog.Printf("[启动日志] DNS定时任务服务启动失败: %v", err)
			zap.L().Error("Failed to start DNS cron service", zap.Error(err))
		} else {
			stdlog.Println("[启动日志] DNS定时任务服务启动成功")
		}
	}()

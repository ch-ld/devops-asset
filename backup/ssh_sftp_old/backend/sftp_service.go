...
func (s *SftpService) selectTargetIP(host *model.Host, preferredType string) (string, error) {
	// 解析公网IP
	...
	// 过滤空字符串
	publicIPs = s.filterEmptyStrings(publicIPs)
	privateIPs = s.filterEmptyStrings(privateIPs)
...
func (s *SftpService) filterEmptyStrings(strs []string) []string {
	var result []string
